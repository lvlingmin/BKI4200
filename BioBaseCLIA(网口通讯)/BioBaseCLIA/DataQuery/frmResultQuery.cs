...
namespace BioBaseCLIA.DataQuery
{
    public partial class frmResultQuery : frmParent
    {
            ...
            switch (CommunicationType)
            {
                case "网口通讯":
                    ...
                    if (LisCommunication.Instance.IsConnect())
                    {
                        ...
                        frmMsgShow.MessageShow("信息提示！","系统未连接LIS服务器！");
                    #endregion
                    break;
                case "串口通讯":
                    #region 串口通讯批量发送实验结果
                    if (LisConnection.Instance.IsOpen())
                    {
                        ...
                        {
                            frmMsgShow.MessageShow("信息提示！","串口正在进行数据通讯！请稍后再进行发送！");
                            return;
                        }
                        DataTable dtshow = new DataTable();
                        DataColumn dc = new DataColumn("SampleNo", typeof(string));
                        ...
                        frmMsgShow.MessageShow("信息提示！","串口未打开！");
                    #endregion
                    break;
                default:
                    break;
        ...
        private void btnImPort_Click(object sender, EventArgs e)
        {
            ...
            dialog.Filter = "Excel xls文件|*.xls|Excel xlsx文件|*.xlsx";
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (dt.Rows.Count == 0)
                {
                    ...
                    f.MessageShow("导入提示", "导入的数据为空！"+filePath+dt);
                    return;
                }
                DataTable _dtData = (DataTable)dgvSampleData.DataSource;  //将控件内容导入内存表_dtData
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (_dtData == null)     
                    {
                        ...
                        dgvSampleData.Rows.Add(dgvSampleData.Rows.Count, 1);    //第一个参数位置添加新行，用1填充。
                    ...
                    {
                        DataRow newrow = _dtData.NewRow();   //在控件dgvSampleData中新建一行用来输入数据
                ...
                if (_dtData == null)
                    dgvSampleData.DataSource = _dtData;    //将导好的内存表导入控件
        ...
        private void btnExPort_Click(object sender, EventArgs e)
        {
            ...
            //string fFlag = dialog.FileName.Substring(dialog.FileName.LastIndexOf(".") + 1);  //lyqadd,20190801,获取扩展名，是xls 或 xlsx
            //dialog.FileName = DateTime.Now.ToString("yyyyMMdd") + "_实验结果";
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                for (int i = 0; i < dgvSampleData.Rows.Count; i++)
                {
                    if (!dgvSampleData.Rows[i].Selected) continue;    //检查是第几行被选中
                    ...
                    for (int j = 3; j < dgvSampleData.Columns.Count - 2; j++)
                    {
                        ...
                        */
                        //lyq add 20190802，导出时，针对发光值这一列转换为double类型
        ...
        void control_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (dgvSampleData.CurrentCell.ColumnIndex == 6 || dgvSampleData.CurrentCell.ColumnIndex == 7)
            {
                ...
                {
                    if (dgvSampleData.CurrentCell.ColumnIndex == 7 && e.KeyChar == 46)
                    {
                        if (dgvSampleData.CurrentCell.Value.ToString().IndexOf('.') > -1)
                        {
                            e.Handled = true;
                            return;
                        }
        ...
        private void fbtnTestAgain_Click(object sender, EventArgs e)
        {
            ...
            {
                if (Convert.ToInt32(_dtSP.Rows[0]["Status"]) == 2 || Convert.ToDateTime(_dtSP.Rows[0]["SendDateTime"])<DateTime.Now.Date)
                    frmMsgShow.MessageShow("信息提示", "样本已经卸载！");
