...
namespace BioBaseCLIA.DataQuery
{
    public partial class frmSupplyStatus : frmParent
    {
        ...
        public void frmSupplyStatus_Load(object sender=null, EventArgs e=null)
        {
            ...
            GetSupplyInfo();
            int width =Convert.ToInt32( dgvReagentInfo.Size.Width / 5);
            dgvReagentInfo.Columns[0].Width = Convert.ToInt32(width - width * 0.1) - 1;
            dgvReagentInfo.Columns[1].Width = width;
            dgvReagentInfo.Columns[2].Width = Convert.ToInt32( width + width*0.3);
            dgvReagentInfo.Columns[3].Width = Convert.ToInt32(width - width * 0.1)-1;
            dgvReagentInfo.Columns[4].Width = Convert.ToInt32(width - width * 0.1) - 1;
        ...
        private void GetSupplyInfo()
        {
            ...
            dv.Sort = "Postion";
            dtRg = dv.ToTable();
            dgvReagentInfo.DataSource = dtRg;         
        ...
        private void subBottle2_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                if (!CheckFormIsOpen("frmLoadSu"))
