...
namespace BioBaseCLIA.SysMaintenance
{
    public partial class frmInstruGroupTest : frmParent
    {
                        ...
                        if (rdbtnTubeRack.Checked)
                        {
                            ...
                            if (rdbtnReactTrayp.Checked)
                            {
                                ...
                                NetCom3.Instance.Send(NetCom3.Cover("EB 90 31 01 01 " + plate.ToString("x2") + " " + column.ToString("x2")
                                    + " " + hole.ToString("x2") + " " + putTubePos.ToString("x2")), 1);
                    ...
                    while (LoopNum > 0)
                    {
                        if (TubeNum <= 80)
                        {
                            ...
                            if (pos == "0")
                            {
                                ...
                                int hole = int.Parse(takeTubePos) % 11 == 0 ? 11 : int.Parse(takeTubePos) % 11;
                                NetCom3.Instance.Send(NetCom3.Cover("EB 90 31 01 01 " + plate.ToString("x2") + " " + column.ToString("x2")
                                           + " " + hole.ToString("x2") + " " + pos1.ToString("x2")), 1);
                                if (!NetCom3.Instance.MoveQuery() && NetCom3.Instance.MoverrorFlag != (int)ErrorState.IsNull)
                                {
                                    InitControlEnable();
                                    return;
                        ...
                        //取样位置++
                        InPos = InPos + 1 == 81 ? 1 : InPos + 1;
                        takeSamplePos = takeSamplePos + 1 == 61 ? 1 : takeSamplePos + 1;
                        LoopNum--;
                        BeginInvoke(new Action(() =>
                   {
                ...
                if (WashTrayFlag)
                {
                    ...
                    while (LoopNum > 0)
                    {
                        ...
                        if (chbRead.Checked)
                        {
                            ...
                            {
                                //清洗盘逆时针旋转7位到读数位置
                                NetCom3.Instance.Send(NetCom3.Cover("EB 90 31 03 01 07"), 2);
                                if (!NetCom3.Instance.WashQuery())
                                {
                                    InitControlEnable();
                                    return;
        ...
        bool MoveTestCondition()
        {
            ...
            if (chbTakeTube.Checked)
            {
                if (rdbtnTubeRack.Checked)
                {
                    ...
                    if (sumTubeNum < int.Parse(txtTubeLoopNum.Text))
                    {
        ...
        void reactTrayTubeClear()
        {
            ...
            BeginInvoke(DLInfo, new object[] { DateTime.Now.ToString("HH-mm-ss") + ": " + "正在清空温育盘。。" });
            ...
            BeginInvoke(DLInfo, new object[] { DateTime.Now.ToString("HH-mm-ss") + ": " + "清空温育盘完成。" });
        }
        ...
        private void NewWashStar_Click(object sender, EventArgs e)
        {
            ...
            {
                frmMsgShow.MessageShow("信息提示", "参数不正确");
                return;
            }
            if (flow[2] && !flow[3])
            {
            ...
            if (comboBox1.SelectedItem.ToString() != "")
            {
                ...
                if (int.Parse(LeftCount1) < numericUpDown3.Value)
                {
                    frmMsgShow.MessageShow("信息提示", "底物" + whichPipe + "可使用量不足，请及时进行更换。");
                    return;
                }
            }
            ...
            if (flow[0])//清空
            {
                ...
                for (int i = 0; i < 30; i++)
                {
                    ...
                    if (!NetCom3.Instance.MoveQuery() && NetCom3.Instance.MoverrorFlag != (int)ErrorState.IsNull)
                    {
                        NewWashEnd();  // 关掉标志位 取消订阅函数：打印 HHMMSS PMT背景值
                        return;
                    }
                    NetCom3.Instance.Send(NetCom3.Cover("EB 90 31 03 01 " + (-1).ToString("X2").Substring(6, 2)), 2);
                    //逆时针计数。  Jun 2019/1/22
            ...
            for (int i = 0; i < 30 + tubeNum; i++)
            {
                ...
                if (temptubenum > 0)//夹新管
                {
                    if (flow[1])
                    {
                            ...
                            if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                            {
                                ...
                                if (iNeedCool < 12)
                                {
                                    if (iNeedCool == 3 || iNeedCool == 6 || iNeedCool == 9)
                                    {
                                        ...
                                        takeTubePos = BoardNextPos(int.Parse(takeTubePos), true, out isMoevtoNextBoard).ToString();
                                    }
                                    else
                                    {
                                        isMoevtoNextBoard = false;
                                        takeTubePos = BoardNextPos(int.Parse(takeTubePos), false, out isMoevtoNextBoard).ToString();
                                    }

                                    plate = int.Parse(takeTubePos) % 88 == 0 ? int.Parse(takeTubePos) / 88 - 1 : int.Parse(takeTubePos) / 88;//几号板
                                    column = int.Parse(takeTubePos) % 11 == 0 ? int.Parse(takeTubePos) / 11 - (plate * 8) : int.Parse(takeTubePos) / 11 + 1 - (plate * 8);
                                    hole = int.Parse(takeTubePos) % 11 == 0 ? 11 : int.Parse(takeTubePos) % 11;
                                    goto AgainNewMove;
                                }
                                else
                                {
                        ...
                        //那个架子减了一个管
                        OperateIniFile.WriteIniData("Tube", "Pos" + (plate + 1), (88 - (column - 1) * 11 - hole).ToString(), iniPathSubstrateTube);
                        OperateIniFile.WriteIniData("Tube", "TubePos", nextpos.ToString(), iniPathSubstrateTube);
                    }
        ...
        private void NewWashEnd_Click(object sender, EventArgs e)
        {
            ...
            {
                frmMsgShow.MessageShow("信息提示","已经准备终止或者实验没有在运行。");
                return;
            }
            CancellationToken = true;
        }
        ...
        private void functionButton1_Click(object sender, EventArgs e)
        {
            if (isNewWashRun)
            {
                frmMsgShow.MessageShow("信息提示","请在仪器空闲时执行此操作");
                return;
            }
            ...
            if (basenum > (leftcount1 + leftcount2))
            {
                frmMsgShow.MessageShow("信息提示", "底物不足，请及时更换！");
                return;
            }
            ...
            for (int i = 0; i < basenum; i++)
            {
                ...
                if (leftcount1 > 0)//substrateNum1
                {
                    if (leftcount1 > leftcount2 && leftcount2 > 0)
                        substratePipe = "01";
                    else
                        substratePipe = "10";
                }
                ...
                NetCom3.Instance.Send(NetCom3.Cover("EB 90 11 0A 00 " + substratePipe + ""), 5);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        OperateIniFile.WriteIniData("Substrate2", "LeftCount", leftcount2.ToString(), iniPathSubstrateTube);
                    }
        ...
        public void CleanAllReactTray()
        {
            ...
            /// </summary>
            string iniPathReactTrayInfo = Directory.GetCurrentDirectory() + "\\ReactTrayInfo.ini";
            Run.frmWorkList.TrayRemoveAllTube = true;
            for (int i = 1; i < 81; i++)
            {
        ...
        void CleanTrayWash(int oneOrTwo)
        {
            ...
            if (oneOrTwo == 1)
            {
                ...
                order = "EB 90 31 03 03 00 11 11 " + substratePipe + "0";
            }
            else if (oneOrTwo == 2)
            {
        ...
        private bool RemoveTubeOutCleanTray()
        {
            ...
            int IsKnockedCool = 0;
        AgainNewMove:
            if (isNewWashEnd()) return false; //lyq add 20190822 结束标志
            NetCom3.Instance.Send(NetCom3.Cover("EB 90 31 01 04 06"), 1);
            if (!NetCom3.Instance.MoveQuery())
            {
                #region 发生异常处理
