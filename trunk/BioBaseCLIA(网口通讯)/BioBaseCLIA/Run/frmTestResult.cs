...
namespace BioBaseCLIA.Run
{
    public partial class frmTestResult : frmParent
    {
        ...
        /// 实验结果表的bll实例
        /// </summary>
        private BLL.tbAssayResult bllAssayResult = new BLL.tbAssayResult();

        /// <summary>
        /// 质控结果表的bll实例
        /// </summary>
        private BLL.tbQCResult bllQCResult = new BLL.tbQCResult();
        ...
        /// </summary>
        private BLL.tbSampleInfo bllsp = new BLL.tbSampleInfo();

        private frmMessageShow frmMsgShow = new frmMessageShow();

        /// <summary>
        /// 实验结果表的model实例
        /// </summary>
        private Model.tbAssayResult modelAssayResult = new Model.tbAssayResult();

        /// <summary>
        /// 质控结果表的model实例
        /// </summary>
        private Model.tbQCResult modelQCResult = new Model.tbQCResult();
        ...
        /// </summary>
        private Model.tbSampleInfo modelSp = new Model.tbSampleInfo();

        #endregion 变量和实例

        public static bool BRun = true;
        private CalculateCurve.Calculater er = null;//2018-07-23 zlx add

        /// <summary>
        /// 底物与管架配置文件地址 2018-10-17 zlx add
        /// </summary>
        private string iniPathSubstrateTube = Directory.GetCurrentDirectory() + "\\SubstrateTube.ini";

        //2018-08-15 zlx add
        private List<int> OverSubpos = new List<int>();//2018-10-17
        ...
        /// <summary> 将BindingList类型转换为IList类型 </summary> <returns>返回IList<TestResult>的值</returns>
        ...
        private void fbtnTestAgain_Click(object sender, EventArgs e)
        {
            ...
            if (bUpdate > 0)
                frmMsgShow.MessageShow("信息提示", "重测设置成功！");
        }
        ...
        private void fbtnExportData_Click(object sender, EventArgs e)
        {
            ...
            //    f.MessageShow("导出提示", "实验结果不存在，无法导出！");
            //    return;
            //}
            //string FileName = DateTime.Now.ToString("yyyyMMdd") + "_实验结果";
            //string saveFileName = "";
            ...
            ////写入标题
            //for (int i = 0; i < dgvResultData.ColumnCount; i++)
            //{
            //    worksheet.Cells[1, i + 1] = dgvResultData.Columns[i].HeaderText;
            //}
            ////写入数值
            //for (int r = 0; r < dgvResultData.Rows.Count; r++)
            //{
            //    for (int i = 0; i < dgvResultData.ColumnCount; i++)
            //    {
            //        worksheet.Cells[r + 2, i + 1] = dgvResultData.Rows[r].Cells[i].Value;
            //    }
