...
namespace BioBaseCLIA.Run
{
    public partial class frmWorkList : frmParent
    {
        ...
        Action<string, int> TestWashInfo;
        /// <summary>
        /// 正在做的实验步骤
        /// </summary>
        ...
        /// 加试剂2时间
        /// </summary>
        int Regent2Time = int.Parse(OperateIniFile.ReadInIPara("Time", "Regent2Time"));
        /// <summary>
        /// 加试剂3时间
        /// </summary>
        int Regent3Time = int.Parse(OperateIniFile.ReadInIPara("Time", "Regent3Time"));
        /// <summary>
        /// 单独加试剂时间
        /// </summary>
        int SingleRegentTime = int.Parse(OperateIniFile.ReadInIPara("Time", "SingleRegentTime"));
        /// <summary>
        /// 加磁珠时间
        /// </summary>
        ...
        /// 向温育盘加样本位置顺序
        /// </summary>
        private List<int> TrayTubeOrder = new List<int>();
        public frmWorkList()
        {
            InitializeComponent();
        ...
        private void frmWorkList_Load(object sender, EventArgs e)
        {
            ...
            if (!NetCom3.isConnect)
            {
                if (NetCom3.Instance.CheckMyIp_Port_Link())
                {
                    ...
                    //if (!NetCom3.isConnect)
                    //return;
                    //this delete y 20180512 虽然代码一般不会执行到这里，因为联机检测返回true但却网络连接不上的情况太少。不过还是打算注释掉此处，避免从这里return导致后面的事件没有附加上方法。
                }
            }
        ...
        void EmergencySampleSch()
        {
            ...
            if (RunFlag == (int)RunFlagStart.IsRuning)
            {
                ...
                if (EmergencyFlag)
                {
                    ...
                    List<string> runFreeTime = new List<string>() { (sumTime + 2).ToString() + "-100000" };
                    runFreeTime = GetFreeTime(RunSchedule);
                    #endregion
                    //未运行的样本进度计算
            ...
            else
            {
                ...
                //进度计算
                lisTestSchedule = ExperimentalScheduleAlgorithm(lisTestSchedule, new List<string>() { "0-100000" });
                lisTestSchedule.Sort(new SortRun());
            }
            #endregion
        ...
        void SetStopWatch()//实验倒计时,初始化并开始
        {
            ...
            {
                TimeLabel2.Text = span.Hours.ToString("00");
            ...
            for (int i = 0; i < dtSampleInfo.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < int.Parse(dtSampleInfo.Rows[i]["RepeatCount"].ToString()); j++)
                {
                    ...
                    else
                    {
                        ...
                        //2019-02-28 zlx mod
                        if (rows.Length > 0 && !(rows[0]["SampleType"].ToString().Contains("标准品") || rows[0]["SampleType"].ToString().Contains("质控品") || rows[0]["SampleType"].ToString().Contains("校准品")))
                        {
                            //获取稀释倍数
                    ...
                    switch (lisTM[i].StepName)
                    {
                        ...
                        case ExperimentStep.AddRegent1:
                            if (lisTM[i].ProMethod == 2)
                            {
                                ...
                                testschedule.AddLiqud = lisTM[i].AddLiqud;
                                testschedule.StartTime = 0;
                                testschedule.TimeLengh = SingleRegentTime;
                                testschedule.stepNum = stepTime++;
                                testschedule.EndTime = testschedule.StartTime + testschedule.TimeLengh;
        ...
        public List<TestSchedule> orderModifyTestID(List<TestSchedule> lisTS)
        {
            ...
            string FirstPos = "";
            //2018-08-29 zlx mod
            DataRow[] dr = dtReactTrayInfo.Select("Pos='no80'");
            ...
            foreach (TestSchedule Ts in tempTestSchedule)
            {
                ...
                //TrayTubeOrder.Add(Ts.AddSamplePos);
            }
            #endregion
            ...
            for (int i = 0; i < lisTS.Count; i++)
            {
                ...
                switch (lisTS[i].TestScheduleStep)
                {
                    case TestSchedule.ExperimentScheduleStep.AddLiquidTube:
                        ...
                        testSchedule.EndTime = testSchedule.StartTime + testSchedule.TimeLengh;
                        //testSchedule.TimeLengh = testSchedule.EndTime - testSchedule.StartTime;                               
        ...
        public List<int> CountAddLiquidTime(List<TestSchedule> lisTS, List<string> freetime)
        {
            int toWashTime = 0;
            List<string> freeTime = new List<string>(freetime.ToArray());
            //freeTime = freetime;
            int beforeTime = 0;
            ...
            for (int i = 0; i < lisTS.Count; i++)
            {
                ...
                switch (lisTS[i].TestScheduleStep)
                {
                    case TestSchedule.ExperimentScheduleStep.AddLiquidTube:
                        ...
                        int addLiquidStartTime = AllowTime(freeTime, lisTestid);
                        LogFile.Instance.Write("TESTID " + testSchedule.TestID + " 开始时间  ： " + addLiquidStartTime);
                        listTime.Add(addLiquidStartTime);
                        testSchedule.StartTime = addLiquidStartTime;
                        testSchedule.EndTime = testSchedule.StartTime + testSchedule.TimeLengh;
                        testSchedule.TimeLengh = testSchedule.EndTime - testSchedule.StartTime;
                        beforeTime = testSchedule.EndTime;
                    ...
                    case TestSchedule.ExperimentScheduleStep.Wash1:
                        ...
                        testSchedule.TimeLengh = testSchedule.EndTime - testSchedule.StartTime;
                        beforeTime = testSchedule.EndTime;
                        ChangFreeTime(freeTime, testSchedule.StartTime, 15);
                        break;
                    case TestSchedule.ExperimentScheduleStep.WashTray:
                        testSchedule.StartTime = beforeTime;
                        testSchedule.EndTime = testSchedule.StartTime + testSchedule.TimeLengh;
                        testSchedule.TimeLengh = testSchedule.EndTime - testSchedule.StartTime;
                        beforeTime = testSchedule.EndTime;
                        toWashTime = testSchedule.StartTime;
                        LogFile.Instance.Write("TESTID " + testSchedule.TestID + " 结束时间  ： " + testSchedule.StartTime);
                        ChangFreeTime(freeTime, testSchedule.StartTime, 15);
                        break;
                }
            }
                ...
                switch (lisTS[index].TestScheduleStep)
                {
                    ...
                    case TestSchedule.ExperimentScheduleStep.AddBeads:
                        for (int i = 0; i < freeTime.Count; i++)
                        {
                            ...
                            int maxTime = int.Parse(minMaxTime[1]);
                            if (
                                beforeTime >= minTime && beforeTime + lisTS[index].TimeLengh <= maxTime)
                            {
                                isAllowFlag = true;
                                break;
                            }
        ...
        /// <returns></returns>
        public List<string> GetTimeRequired(List<TestSchedule> temp)
        {
            int beforeTime = 0;
            List<string> ls = new List<string>();
            for (int i = 0; i < temp.Count; i++)
            {
                switch (temp[i].TestScheduleStep)
                {
                    case TestSchedule.ExperimentScheduleStep.AddLiquidTube:
                        ls.Add(0.ToString() + "-" + temp[i].TimeLengh);
                        beforeTime = temp[i].TimeLengh;
                        break;
                    case TestSchedule.ExperimentScheduleStep.AddBeads:
                        ls.Add((beforeTime) + "-" + temp[i].TimeLengh);
                        beforeTime = beforeTime + temp[i].TimeLengh;
                        break;
                    case TestSchedule.ExperimentScheduleStep.AddSingleR:
                        ls.Add((beforeTime) + "-" + temp[i].TimeLengh);
                        beforeTime = beforeTime + temp[i].TimeLengh;
        ...
        void BindData(List<TestSchedule> lisTS, int nostartid)
        {
            ...
            DataTable dtSample = bllsp.GetList("Status = 0 and tbSampleInfo.SendDateTime >=#"
                                     + DateTime.Now.ToString("yyyy-MM-dd") + "#and tbSampleInfo.SendDateTime <#"
                                     + DateTime.Now.AddDays(1).ToString("yyyy-MM-dd") + "#").Tables[0];
            for (int i = 0; i < dtSample.Rows.Count; i++)
            {
                LogFile.Instance.Write(dtSample.Rows[i]["SampleType"].ToString());
            }
            for (int i = nostartid; i <= lisTS[lisTS.Count - 1].TestID; i++)
            {
                ...
                //频繁操作数据库浪费了大量的时间，改成一次查出来，然后从DataTable中查数据 20190708 gongjun
        ...
        void buttonEnableRun(bool Flag)
        {
            ...
            btnLoadSample.Enabled = fbtnReturn.Enabled = !Flag;//2018-11-29 zlx mod
        }
        #region 实验方法
        /// <summary>
        ...
        void TestRun(object sender, EventArgs e)
        {
            ...
            if (frmMain.LackLq[0] > 0 || frmMain.LackLq[1] > 0 || frmMain.LackLq[2] > 0 || frmMain.LackLq[3] > 0)
            {
                ...
                {
                    st.Append("\n实验无法开始，请添加清洗液！");
                    frmMsgShow.MessageShow("缺液警告", st.ToString());
                ...
                {
                    st.Append("\n将会影响到此次实验,请确认是否继续？");
                    DialogResult dr = frmMsgShow.MessageShow("缺液警告", st.ToString());
            ...
            TestWashInfo = new Action<string, int>((str, index) =>
            {
                dgvWorkListData.Rows[index - 1].Cells["SampleNo"].Value = dgvWorkListData.Rows[index - 1].Cells["SampleNo"].Value + "-" + str;
            });//实验的实时运行实验状态
        ...
        bool MachineInit()
        {
            ...
            //if (!CleanTrayWashPipeline()) return false;//每日只需要一次清洗，在组合测试中进行
            if (RunFlag != (int)RunFlagStart.IsRuning) return false;
            reactTrayTubeClear();
            if (RunFlag != (int)RunFlagStart.IsRuning) return false;
            #endregion
                ...
                if (ReacTrayTub != "")
                {
                    ...
                    foreach (string ReacTub in ReacTrayTubSplit)
                    {
                        ...
                        if (ReacTub != "" && (OperateIniFile.ReadIniData("ReactTrayInfo", "No" + ReacTub, "", iniPathReactTrayInfo)) == "0")
                        {
                            ...
                            rackToReact(OnePos, int.Parse(ReacTub));
                        }
                    }
                }
            }
            ...
            if (sumReactTubeNum < toUsedTube)
            {
                ...
                for (int i = 0; i < LackTubeNum; i++)
                {
                    ...
                    lock (locker2)
                    {
                        ...
                        LogFile.Instance.Write("时间 ：" + DateTime.Now.ToString("HH-mm-ss") + "  取管位置 ：" + moveTube1.TakeTubePos + "  放管位置 ：" + moveTube1.putTubePos);
        ...
        bool RunConditionJudge()
        {
            ...
            //查询试剂信息
            List<ReagentIniInfo> templisRIinfo = QueryReagentIniInfo();
            List<ReagentIniInfo> lisRIinfo = new List<ReagentIniInfo>();
            //foreach (var v in templisRIinfo)
            //{
            //    foreach (var tempItem in lisItem)
            //    {
            //        if (tempItem.RegentBatch == v.BatchNum && !lisRIinfo.Contains(v))
            //        {
            //            lisRIinfo.Add(v);
            //        }
            //    }
            ...
            {
                foreach (var tempRegion in templisRIinfo.FindAll(x =>x.ItemName == v.Key))
                {
                    if (!lisRIinfo.Contains(tempRegion))
                    {
                        lisRIinfo.Add(tempRegion);
                    }
                }
            }
                //所有的标准品或定标液
            ...
            if (Convert.ToDateTime(ValidDate1) < DateTime.Now.Date || Convert.ToDateTime(ValidDate2) < DateTime.Now.Date)
            {
                ...
                DialogResult r = frmMsgShow.MessageShow("底物检查信息", "" + OverInfo + "已过期需更换。本次仍然继续测试？");
            ...
            for (int i = 0; i < 30; i++)
            {
                ...
                if (i != 0)
                {
                    ...
                    if (!NetCom3.Instance.WashQuery())
                    {
                        ...
                        else if (NetCom3.Instance.WasherrorFlag == (int)ErrorState.OverTime)
                        {
                            ...
                            LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "清洗指令接收数据超时！");
                ...
                if (!NetCom3.Instance.MoveQuery())
                {
                    ...
                    else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                    {
                        ...
                        {
                            setmainformbutten();
                            LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在清洗盘扔废管时发生撞管！撞管位置为" + tubeHoleNum);
            ...
            if (!NetCom3.Instance.MoveQuery())
            {
                ...
                if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                {
                    ...
                    else if (iNeedCool < 11)
                    {
                        ...
                        plate = boardPos % 88 == 0 ? boardPos / 88 - 1 : boardPos / 88;//几号板
                        column = boardPos % 11 == 0 ? boardPos / 11 - (plate * 8) : boardPos / 11 + 1 - (plate * 8);
                        hole = boardPos % 11 == 0 ? 11 : boardPos % 11;
                        string num = OperateIniFile.ReadIniData("Tube", "Pos" + (plate + 1), "0", iniPathSubstrateTube);
                        if (int.Parse(num) == 0)
                        {
        ...
        private bool RemoveTubeOutCleanTray()
        {
            ...
            if (!NetCom3.Instance.MoveQuery())
            {
                ...
                if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                {
                    ...
                    {
                        setmainformbutten();
                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在清洗盘扔废管时多次抓空！孔位置为" + tubeHoleNum);
                    ...
                    if (int.Parse(dtInTrayIni.Rows[i][1].ToString()) >= 1)
                    {
                        ...
                        if (!NetCom3.Instance.MoveQuery())
                        {
                            ...
                            else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                            {
                                ...
                                {
                                    setmainformbutten();
                                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在温育盘扔废管时发生撞管！");
                                    return;
                                }
                            }
                            else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.OverTime)
            ...
            try
            {
                if (lisTestSchedule.Count > 0)
                {
                    ...
                    while (SampleNumCurrent > StopList.Count && RunFlag == (int)RunFlagStart.IsRuning)
                    {
                        ...
                        {
                            if ((frmMain.StopFlag[0] || frmMain.StopFlag[1] || frmMain.StopFlag[2]) || BFullReactTray)
                            {
                                LogFile.Instance.Write("**********" + " if ((frmMain.StopFlag[0] || frmMain.StopFlag[1] || frmMain.StopFlag[2]) || BFullReactTray) " + "**********");
                                AllPause();
                                GetNoStartList();
                            }
                            if (TubeStop)
                            {
                                if (!frmMain.pauseFlag)
                                {
                                    LogFile.Instance.Write("**********" + " if (TubeStop) " + "**********");
                                    if ((frmMain.StopFlag[0] || frmMain.StopFlag[1] || frmMain.StopFlag[2]) || BFullReactTray)
                                    {
                                        AllPause();
                                        GetNoStartList();
                            ...
                            if (SubstrateStop)
                            {
                                if (!frmMain.pauseFlag)
                                {
                                    LogFile.Instance.Write("**********" + " if (SubstrateStop) " + "**********");
                                    AllPause();
                                    GetNoStartList();
                                }
                                if (substrateNum1 + substrateNum2 > SampleNumCurrent)
                                    SubstrateStop = false;
                            }
                        }
                        else
                        {
                            if ((!frmMain.StopFlag[0] && !frmMain.StopFlag[1] && !frmMain.StopFlag[2]) && StopList.Count > 0 && !TubeStop && !SubstrateStop && !BFullReactTray)
                            {
                                #region 更换管完成，去除缺管状态
                                ...
                                StopList.Clear();
                                Goon();
                                LogFile.Instance.Write("**********" + " StopList.Clear(); " + "**********");
                        ...
                        if (AddTubeStop.Count > 0 && !TubeStop)
                        {
                            ...
                            for (int i = 0; i < AddTubeStopCopy.Count; i++)
                            {
                                ...
                                {
                                    int OnePos = int.Parse(OperateIniFile.ReadIniData("Tube", "TubePos", "1", iniPathSubstrateTube));
                                    rackToReact(OnePos, AddTubeStopCopy[i]);
                        ...
                        switch (TestStep.TestScheduleStep)
                        {
                            ...
                            case TestSchedule.ExperimentScheduleStep.AddLiquidTube:
                                ...
                                foreach (var rowRg in drRg)
                                {
                                    ...
                                    if (int.Parse(TestStep.dilutionTimes) > 1 && int.Parse(diuleftVol[int.Parse(rowRg["Postion"].ToString()) - 1]) - DiuNoUsePro > DiuNum)
                                        IsDiu = false;
                                }
                                if (Rgcounum <= 0 || IsDiu)
                                {
                                ...
                                if (substrateNum1 + substrateNum2 <= Sampleing - completeTestNums)
                                {
                                    SubstrateStop = true;
                                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "底物缺少，将停止加样！");
                                    setmainformbutten();
                                    break;
                                }
                                int movepos = TestStep.AddSamplePos + 15;
                                if (movepos > 80)
                                    movepos = movepos - 80 + 3;
                                ...
                                {
                                    //if (!AddTubeStop.Contains(int.Parse(ss)))
                                    //    AddTubeStop.Add(int.Parse(ss));
                            ...
                            case TestSchedule.ExperimentScheduleStep.Wash1:
                                ...
                                lock (locker2)
                                {
                                    ...
                                    LogFile.Instance.Write("时间 ：" + DateTime.Now.ToString("HH-mm-ss") + "  取管位置 ：" + moveTube4.TakeTubePos + "  放管位置 ：" + moveTube4.putTubePos);
                                }
                                ValueFlag3 = false;
                                if (FirstTubeWash)
                                {
                            ...
                            case TestSchedule.ExperimentScheduleStep.WashTray:
                                ...
                                lock (locker2)
                                {
                                    ...
                                    LogFile.Instance.Write("时间 ：" + DateTime.Now.ToString("HH-mm-ss") + "  取管位置 ：" + moveTube5.TakeTubePos + "  放管位置 ：" + moveTube5.putTubePos);
                                }
                                ValueFlag4 = false;
                                if (FirstTubeWash)
                                {
                    ...
                    while (RunFlag == (int)RunFlagStart.IsRuning)
                    {
                        ...
                        {
                            RunFlag = (int)RunFlagStart.IsStoping;
                            fbtnReturn.Enabled = true;//2018-07-26 zlx add
                            ...
                            {
                                if (frmMain.StopFlag[0] || frmMain.StopFlag[1] || frmMain.StopFlag[2])
                                    frmMsgShow.MessageShow("实验状态", "仪器缺液，部分实验已完成！详细信息请在日志查看！");
                                else
                                    frmMsgShow.MessageShow("实验状态", "供应品缺少，部分实验已完成！详细信息请在日志查看！");
                            }
                            else
                                frmMsgShow.MessageShow("实验状态", "实验完成！");//2018-07-13 zlx mod
                            RunFlag = (int)RunFlagStart.Stoped;
                            RunLightFlag = false;
                            buttonEnableRun(false);
                            frmMain.pauseFlag = false;
        ...
        private void addLiquid(object testS)
        {
            ...
            drRg = dtRgInfo.Select("RgName='" + testTempS.ItemName.ToString() +
                "' AND leftoverTestR1>0 ").OrderBy(x => int.Parse(x["leftoverTestR1"].ToString())).ToArray();
            int rgPos = 0;
            int rgindex = -1;
            ...
            switch (testTempS.TestScheduleStep)
            {
                case TestSchedule.ExperimentScheduleStep.AddLiquidTube:
                    ...
                    for (int j = 0; j < TestStepSingle.Length; j++)
                    {
                            ...
                            if (DiluteName != "1")
                            {
                                ...
                                for (int i = 0; i < diuList.Count; i++)
                                {
                                    ...
                                    if (rgPos > 0)
                                    {
                                        ...
                                        #region 夹新管到反应盘
                                        while (MoveTubeUseFlag)//2018-08-07 zlx add
                                            NetCom3.Delay(10);
                                        //管架夹新管未夹位置统计
                                        int tubeNum = 0;
                                        for (int jj = 0; jj < lisMoveTube.Count; jj++)
                                        {
                                            if (lisMoveTube[jj].TakeTubePos.Split('-')[0] == "0")
                                            {
                                                tubeNum++;
                                            }
                                        }
                                        int OnePos = int.Parse(OperateIniFile.ReadIniData("Tube", "TubePos", "1", iniPathSubstrateTube)) + tubeNum;
                                        rackToReact(OnePos, pos);
                                        if (TubeStop)//加新管时管架为空 2019-02-22
                                        {
                                            RemoveTestList(testTempS);
                                        ...
                                        #endregion
                                        #region 加稀释液
                                        //剩余稀释液体积 lyn add 20180611
                                        string leftdiuVol = (int.Parse(diuleftVol[rgPos - 1])).ToString("x4");
                                        ///吸取试剂盘（rgPos）位置稀释液加到反应盘（pos）位置
                                        AddErrorCount = 0;
                                        NetCom3.Instance.Send(NetCom3.Cover("EB 90 31 02 06 " + rgPos.ToString("x2") + " " + pos.ToString("x2")
                                            + " " + DiuVol.ToString("x2") + " " + leftdiuVol.Substring(0, 2) + " " + leftdiuVol.Substring(2, 2)), 0);
                                        //指令未执行完成进行等待
                                        ...
                                        if (!NetCom3.Instance.SPQuery())
                                        {
                                            ...
                                            if (NetCom3.Instance.AdderrorFlag == (int)ErrorState.IsKnocked && AddErrorCount < 2)
                                            {
                                                ...
                                                //重新发送指令
                                                if (againSend == "")
                                                    againSend = "EB 90 31 02 06 " + rgPos.ToString("x2") + " " + pos.ToString("x2")
                                                    + " 00 " + leftdiuVol.Substring(0, 2) + " " + leftdiuVol.Substring(2, 2);
                                            }
                                            else if (NetCom3.Instance.AdderrorFlag == (int)ErrorState.Sendfailure)
                                            {
                                                ...
                                                if (againSend == "")
                                                    againSend = "EB 90 31 02 06 " + rgPos.ToString("x2") + " " + pos.ToString("x2")
                                                    + " " + DiuVol.ToString("x2") + " " + leftdiuVol.Substring(0, 2) + " " + leftdiuVol.Substring(2, 2);
                                            }
                                            else if (NetCom3.Instance.AdderrorFlag == (int)ErrorState.OverTime)
                                            {
                                                NetCom3.Instance.stopsendFlag = true;
                                                LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加稀释液指令接收超时！");//从试剂位"+ rgPos + "向"+pos+"位置加稀释液指令接收数据超时
                                        ...
                                        diuleftVol[rgPos - 1] = (int.Parse(diuleftVol[rgPos - 1]) - (int)(DiuVol + abanDiuPro)).ToString();
                                        OperateIniFile.WriteIniData("ReagentPos" + rgPos.ToString(), "leftDiuVol", diuleftVol[rgPos - 1], iniPathReagentTrayInfo);
                                        #endregion
                                        if (AddErrorCount > 0)
                                        {
                                            if (AddErrorCount > 1)
                                            {
                                                setmainformbutten();
                                                NetCom3.Instance.stopsendFlag = true;
                                                LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加稀释液加样针撞针！");
                                        ...
                                        if (!NetCom3.Instance.SPQuery())
                                        {
                                            ...
                                            else if (NetCom3.Instance.AdderrorFlag == (int)ErrorState.OverTime)
                                            {
                                                NetCom3.Instance.stopsendFlag = true;
                                                setmainformbutten();
                                                LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加样本指令接收超时！");
                                                frmMsgShow.MessageShow("加样错误提示", "指令接收超时，实验已终止");
                                                AllStop();
                                                //addLiquiding = false;
                                        ...
                                        if (AddErrorCount > 0)
                                        {
                                            if (AddErrorCount > 1)
                                            {
                                                setmainformbutten();
                                                NetCom3.Instance.stopsendFlag = true;
                                                LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加样本加样针撞针！");
                                                frmMsgShow.MessageShow("加样错误提示", "加样针撞针未能修复，实验已终止");
                                                //addLiquiding = false;
                                                AllStop();
                                            }
                                        ...
                                        while (MoveTubeUseFlag)
                                            Thread.Sleep(30);
                                        NetCom3.Instance.Send(NetCom3.Cover("EB 90 31 01 0B " + pos.ToString("x2")), 1);
                                        if (!NetCom3.Instance.MoveQuery())
                                        {
                                            #region 异常处理
                                            ...
                                            else
                                            {
                                                ...
                                                NetCom3.Instance.stopsendFlag = true;
                                                setmainformbutten();
                                                LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "混匀" + pos + "位置异常！");
                                                frmMsgShow.MessageShow("加样错误提示", "混匀异常！");
                                                //addLiquiding = false;
                                                AllStop();
                                                break;
                                            }
                        ...
                        else if (TestStepSingle[j] == "S")
                        {
                            ...
                            if (testTempS.getSamplePos.Contains("R"))
                            {
                                ...
                                if (!NetCom3.Instance.SPQuery())
                                {
                                  ...
                                  + " 00";
                                    ...
                                    else if (NetCom3.Instance.AdderrorFlag == (int)ErrorState.OverTime)
                                    {
                                        NetCom3.Instance.stopsendFlag = true;
                                        setmainformbutten();
                                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加试剂指令接收超时！");
                                        frmMsgShow.MessageShow("加样错误提示", "指令接收超时，实验已终止");
                                        //addLiquiding = false;
                                        AllStop();
                                    }
                                ...
                                if (AddErrorCount > 0)
                                {
                                    if (AddErrorCount > 1)
                                    {
                                        NetCom3.Instance.stopsendFlag = true;
                                        setmainformbutten();
                                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加试剂撞针！");
                                        frmMsgShow.MessageShow("加样错误提示", "加样针撞针未能修复，实验已终止");
                                        //addLiquiding = false;
                                        AllStop();
                                    }
                            ...
                            else
                            {
                                ...
                                if (!NetCom3.Instance.SPQuery())
                                {
                                  ...
                                  + " 00";
                                    ...
                                    else if (NetCom3.Instance.AdderrorFlag == (int)ErrorState.OverTime)
                                    {
                                        NetCom3.Instance.stopsendFlag = true;
                                        setmainformbutten();
                                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加样本加样指令接收超时！");
                                        frmMsgShow.MessageShow("加样错误提示", "指令接收超时，实验已终止");
                                        //addLiquiding = false;
                                        AllStop();
                                    }
                                ...
                                if (AddErrorCount > 0)
                                {
                                    if (AddErrorCount > 1)
                                    {
                                        ...
                                        setmainformbutten();
                                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加样本加样针撞针！");
                                        frmMsgShow.MessageShow("错误提示", "加样针撞针未能修复，实验已终止");
                                        //addLiquiding = false;
                                        AllStop();
                                    }
                                    else
                            ...
                            lisProBar[testTempS.TestID - 1].BarColor[StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "S")]
                                = Color.Gray;
                            toGray(testTempS.TestID - 1, StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "S"));
                            lisProBar[testTempS.TestID - 1].Invalidate();
                            AddingSampleFlag = false;
                        }
                        else if (TestStepSingle[j] == "R1")
                        {
                            ...
                            if (drRg.Length > 0)
                            {
                                for (int g = 0; g < drRg.Length; g++)
                                {
                                    ...
                                    if (dgvWorkListData.Rows[testTempS.TestID - 1].Cells["SampleType"].Value.ToString().Contains("标准品") || dgvWorkListData.Rows[testTempS.TestID - 1].Cells["SampleType"].Value.ToString().Contains("质控品"))
                                    {
                            ...
                            if (rgPos > 0)
                            {
                                ...
                                if (!NetCom3.Instance.SPQuery())
                                {
                                    ...
                                    else if (NetCom3.Instance.AdderrorFlag == (int)ErrorState.OverTime)
                                    {
                                        NetCom3.Instance.stopsendFlag = true;
                                        setmainformbutten();
                                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加试剂加样指令接收超时！");
                                        frmMsgShow.MessageShow("加样错误提示", "指令接收超时，实验已终止");
                                        //addLiquiding = false;
                                        AllStop();
                                    }
                                ...
                                if (AddErrorCount > 0)
                                {
                                    if (AddErrorCount > 1)
                                    {
                                        NetCom3.Instance.stopsendFlag = true;
                                        setmainformbutten();
                                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加试剂加样针撞针！");
                                        frmMsgShow.MessageShow("加样错误提示", "加样针撞针未能修复，实验已终止");
                                        //addLiquiding = false;
                                        AllStop();
                                    }
                                ...
                                if (TestStepSingle[TestStepSingle.Length - 1] == "R1")
                                {
                                    ...
                                    if (!NetCom3.Instance.MoveQuery())
                                    {
                                        ...
                                        if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.Sendfailure)
                                        {
                                            ...
                                            //重新发送指令
                                            if (againSend == "")
                                                againSend = "EB 90 31 01 0B " + pos.ToString("x2");
                                            if (SendAgain(againSend, 1) == (int)ErrorState.Sendfailure)
                                                goto Again;
                                        }
                                        else
                                        {
                        ...
                        else if (TestStepSingle[j] == "R2")
                        {
                            ...
                            if (rgPos > 0)
                            {
                                ...
                                if (TestStepSingle[TestStepSingle.Length - 1] == "R2")
                                {
                                    while (MoveTubeUseFlag)
                                        Thread.Sleep(30);
                                    NetCom3.Instance.Send(NetCom3.Cover("EB 90 31 01 0B " + pos.ToString("x2")), 1);//add y 20180524混匀
                                    if (!NetCom3.Instance.MoveQuery())//add y 20180524混匀
                                    {
                                        #region 异常处理
                                        string againSend = "";
                                        Again:
                            ...
                            lisProBar[testTempS.TestID - 1].BarColor[StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "R2")]
                                = Color.Gray;
                            toGray(testTempS.TestID - 1, StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "R2"));
                            lisProBar[testTempS.TestID - 1].Invalidate();
                        }
                        else if (TestStepSingle[j] == "R3")
                        {
                            ...
                            lisProBar[testTempS.TestID - 1].BarColor[StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "R3")]
                                = Color.Gray;
                            toGray(testTempS.TestID - 1, StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "R3"));
                            lisProBar[testTempS.TestID - 1].Invalidate();
                        }
                    }
                    #region 夹新管
                ...
                case TestSchedule.ExperimentScheduleStep.AddBeads:
                    lisProBar[testTempS.TestID - 1].BarColor[StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "B") - 1]
                        = Color.Gray;
                    toGray(testTempS.TestID - 1, StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "B") - 1);
                    lisProBar[testTempS.TestID - 1].Invalidate();
                    while (!this.IsHandleCreated)//为了防止出现“在创建窗口句柄之前，不能在控件上调用 Invoke 或 BeginInvoke”的错误
                    {
                        Thread.Sleep(30);
                    ...
                    if (rgPos > 0)
                    {
                        ...
                        if (!NetCom3.Instance.SPQuery())
                        {
                            ...
                            else if (NetCom3.Instance.AdderrorFlag == (int)ErrorState.OverTime)
                            {
                                NetCom3.Instance.stopsendFlag = true;
                                setmainformbutten();
                                LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加磁珠指令接收超时！");
                                frmMsgShow.MessageShow("加样错误提示", "指令接收超时，实验已终止");
                                //addLiquiding = false;
                                AllStop();
                            }
                        ...
                        if (AddErrorCount > 0)
                        {
                            if (AddErrorCount > 1)
                            {
                                NetCom3.Instance.stopsendFlag = true;
                                setmainformbutten();
                                LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "加磁珠加样针撞针！");
                                frmMsgShow.MessageShow("加样错误提示", "加样针撞针未能修复，实验已终止");
                                //addLiquiding = false;
                                AllStop();
                            }
                    ...
                    lisProBar[testTempS.TestID - 1].BarColor[StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "B")]
                        = Color.Gray;
                    toGray(testTempS.TestID - 1, StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "B"));
                    lisProBar[testTempS.TestID - 1].Invalidate();
                    DalayFlag = false;
                    stepTime = 0;
                    break;
                case TestSchedule.ExperimentScheduleStep.AddSingleR:
                    if (testTempS.singleStep == "R1")
                    {
                        ...
                        = Color.Gray;
                        toGray(testTempS.TestID - 1, StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "R1") - 1);
                        lisProBar[testTempS.TestID - 1].Invalidate();
                        while (!this.IsHandleCreated)//为了防止出现“在创建窗口句柄之前，不能在控件上调用 Invoke 或 BeginInvoke”的错误
                        {
                            Thread.Sleep(30);
                    ...
                    else if (testTempS.singleStep == "R2")
                    {
                        ...
                        = Color.Gray;
                        toGray(testTempS.TestID - 1, StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "R2") - 1);
                        lisProBar[testTempS.TestID - 1].Invalidate();
                        while (!this.IsHandleCreated)//为了防止出现“在创建窗口句柄之前，不能在控件上调用 Invoke 或 BeginInvoke”的错误
                        {
                            Thread.Sleep(30);
                        ...
                        //NetCom3.IncubateTray.Reset();//add y 20180524
                        if (rgPos > 0)
                        {
                            //获取当前反应盘可用位置
                            int pos = (testTempS.AddSamplePos) % ReactTrayHoleNum == 0 ? ReactTrayHoleNum : (testTempS.AddSamplePos) % ReactTrayHoleNum;
                            ...
                            string leftR2Vol = (RegentNoUsePro + leftR2 * int.Parse(testTempS.AddLiqud) + leftR2 * abanR2Pro).ToString("x4");//2018-10-13 zlx mod
                                                                                                                                             //string leftR2Vol = (leftR2 * int.Parse(testTempS.AddLiqud) + (int)(int.Parse(testTempS.AddLiqud) * abanR2Pro)).ToString("x4");
                                                                                                                                             ///取试剂盘rgPos位置试剂加到反应盘Pos位置///洗针
                            AddErrorCount = 0;
                            NetCom3.Instance.Send(NetCom3.Cover("EB 90 31 02 04 " + rgPos.ToString("x2") + " " + pos.ToString("x2")
                    ...
                    else if (testTempS.singleStep == "R3")
                    {
                        ...
                        = Color.Gray;
                        toGray(testTempS.TestID - 1, StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "R3") - 1);
                        lisProBar[testTempS.TestID - 1].Invalidate();
                        while (!this.IsHandleCreated)//为了防止出现“在创建窗口句柄之前，不能在控件上调用 Invoke 或 BeginInvoke”的错误
                        {
                            Thread.Sleep(30);
                        ...
                        else
                        {
                            if (drRg.Length > 0)
                            {
                                for (int g = 0; g < drRg.Length; g++)
                                {
                                    //drRg = dtRgInfo.Select("RgName='" + testTempS.ItemName.ToString() + "'");//2018-09-25屏蔽
                                    //标准品、质控品以及其他只能使用它们自身的试剂 2018-08-27 zlx add
                                    if (dgvWorkListData.Rows[testTempS.TestID - 1].Cells["SampleType"].Value.ToString().Contains("标准品") || dgvWorkListData.Rows[testTempS.TestID - 1].Cells["SampleType"].Value.ToString().Contains("质控品"))
                                    {
                                        if (drRg[g]["Batch"].ToString() != dgvWorkListData.Rows[testTempS.TestID - 1].Cells["RegentBatch"].Value.ToString())
                        ...
                        if (rgPos > 0)
                        {
                            ...
                            string leftR3Vol = (RegentNoUsePro + leftR3 * int.Parse(testTempS.AddLiqud) + leftR3 * abanR3Pro).ToString("x4");//2018-10-13 zlx mod
                                                                                                                                             //string leftR3Vol = (leftR3 * int.Parse(testTempS.AddLiqud) + (int)(int.Parse(testTempS.AddLiqud) * abanR3Pro)).ToString("x4");
                                                                                                                                             ///取试剂盘rgPos位置试剂加到反应盘Pos位置///洗针
                            AddErrorCount = 0;
                            NetCom3.Instance.Send(NetCom3.Cover("EB 90 31 02 05 " + rgPos.ToString("x2") + " " + pos.ToString("x2")
        ...
        {
            while (frmSampleLoad.CaculatingFlag || TubeProblemFlag)
            {
                Thread.Sleep(30);
            }
        ...
        /// <returns></returns>
        void rackToReact(int RackPos, int ReactPos)
        {
            LogFile.Instance.Write("**********" + "调用了rackToReact(int RackPos, int ReactPos)=> 管架取管 " + RackPos + " 反应盘放置 " + ReactPos + "**********");
            frmMessageShow frmMsgShow = new frmMessageShow();
            ...
            if (!NetCom3.Instance.MoveQuery())
            {
                ...
                if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                {
                    ...
                    else if (iNeedCool < 11)
                    {
                        ...
                        if (iNeedCool == 3 || iNeedCool == 6 || iNeedCool == 9)
                        {
                            ...
                            RackPos = BoardNextPos(RackPos, true, out isMoevtoNextBoard);
                        }
                        else
                        {
                            isMoevtoNextBoard = false;
                            RackPos = BoardNextPos(RackPos, false, out isMoevtoNextBoard);
                        }
                        //移管手到取管位置takepos[1]（管架）取管
                        plate = RackPos % 88 == 0 ? RackPos / 88 - 1 : RackPos / 88;//几号板
                        column = RackPos % 11 == 0 ? RackPos / 11 - (plate * 8) : RackPos / 11 + 1 - (plate * 8);
                        hole = RackPos % 11 == 0 ? 11 : RackPos % 11;
                        string num = OperateIniFile.ReadIniData("Tube", "Pos" + (plate + 1), "0", iniPathSubstrateTube);
                        if (int.Parse(num) == 0)
                        {
                            iNeedCool = iNeedCool + 3;
                ...
                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                {
                    ...
                    else
                    {
                        ...
                        NetCom3.Instance.stopsendFlag = true;
                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在管架向温育盘抓管时发生撞管！");
                        DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在管架向温育盘抓管时发生撞管，实验将进行停止！");
                        AllStop();
                    }
                }
                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.OverTime)
                {
                    ...
                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在管架向温育盘抓管时接收数据超时！");
                    AllStop();
                    DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在管架向温育盘抓管时接收数据超时，实验将进行停止！");
                }
                #endregion
            }
            else
            ...
            if (!NetCom3.Instance.MoveQuery())
            {
                ...
                if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                {
                    ...
                    else
                    {
                        ...
                        DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在温育盘扔废管时多次抓空，实验将进行停止！");
                        AllStop();
                    }
                }
                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.Sendfailure)
                ...
                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                {
                    ...
                    else
                    {
                        ...
                        NetCom3.Instance.stopsendFlag = true;
                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在温育盘扔废管时发生撞管！");
                        DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在温育盘扔废管时发生撞管，实验将进行停止！");
                        AllStop();
                    }
                }
                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.OverTime)
                {
                    ...
                    NetCom3.Instance.stopsendFlag = true;
                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在温育盘扔废管时接收数据超时！");
                    DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在温育盘扔废管时接收数据超时，实验将进行停止！");
                    AllStop();
                }
                #endregion
            }
        ...
        bool MoveTubeToWash()
        {
            if (lisMoveTube.Count > 0 && lisMoveTube.Exists(ty => ty.putTubePos != null && ty.putTubePos.Split('-')[0] == "2"))
                return true;
            else
                return false;
            //return true;
                    ...
                    else if (Type == 4)//管架抓空
                    {
                        ...
                        if (iNeedCool > 11)
                        {
                            ...
                            DialogResult tempresult = MessageBox.Show("移管手多次抓空，请进行维修,实验已暂停。" + Environment.NewLine
                                + "点击“确定”实验继续，并视为一号管架装载完成。" + Environment.NewLine + "点击“取消”终止实验", "错误"
                        ...
                        else
                        {
                            ...
                            LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "警告" + " *** " + "未读" + " *** " + "管架移管手抓空");
                            setmainformbutten();
                        }
                    }
                }
        ...
        */
        ...
        private void MoveTubeListAddTubeDispose(int pos)//add y 20180727  丢掉不用的管,加液针撞针需要的处理
        {
            ...
            //MoveTubeListAddOne(false);
            //int boardPos = 0;
            //for (int i = 2; i < lisMoveTube.Count; i++)
            //{
            //    string[] stringtemp = lisMoveTube[i].TakeTubePos.Split('-');
            //    if (stringtemp[0] == "0")
            //    {
            //        boardPos = int.Parse(stringtemp[1]);
            //        break;
            //    }
            //}
            //if (boardPos != 0)
            //{
            //    SortMoveTubeList(boardPos + 1, 3);
            //}
            //else
            //{
            //    boardPos = int.Parse(OperateIniFile.ReadIniData("Tube", "TubePos", "1", System.IO.Directory.GetCurrentDirectory() + "\\SubstrateTube.ini"));
            //    OperateIniFile.WriteIniData("Tube", "TubePos", ((boardPos+1)>352 ? 1: (boardPos + 1)).ToString(), iniPathSubstrateTube);
            //}
            lock (locker2)
            {
            ...
            LogFile.Instance.Write("时间 ：" + DateTime.Now.ToString("HH-mm-ss") + "  取管位置 ：" + moveTubeStatus.TakeTubePos + "  放管位置 ：" + moveTubeStatus.putTubePos);
            //lisMoveTube[1].TakeTubePos = "0-" + boardPos;
            //lisMoveTube[1].putTubePos = "1-" + pos;
        }
        private void RemoveTestList(TestSchedule testSchedule)//add y 20180727  整理testschould列表，将进度中撞针的实验设置为跳过，以实现可以继续剩下的实验
        {
            frmMessageShow frmMsgShow = new frmMessageShow();
            string TestnameTemp = testSchedule.TestID.ToString();
            Thread thread;//2019-2-21 zlx mod
            if (TubeStop)
                thread = new Thread(new ThreadStart(() => { frmMsgShow.MessageShow("提示", "编号为:" + TestnameTemp + " 的实验因为稀释缺管，无法继续，已经排除，请在稍后重新运行此实验"); }));
            else
                thread = new Thread(new ThreadStart(() => { frmMsgShow.MessageShow("提示", "编号为:" + TestnameTemp + " 的实验因为发生故障，无法继续，已经排除，请在稍后重新运行此实验"); }));
            if (thread != null)
            {
        ...
        private void RemoveTestList(TestSchedule testSchedule, string endReason)//add y 20180727  整理testschould列表，将进度中撞针的实验设置为跳过，以实现可以继续剩下的实验
        {
            ...
            if (TubeStop)
            {
                thread = new Thread(new ThreadStart(() => { MessageBox.Show("编号为:" + TestnameTemp + " 的实验因稀释缺管已废弃，请稍后重新运行此测试", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information); }));
            ...
            else
            {
                ...
                thread = new Thread(new ThreadStart(() => { MessageBox.Show("编号为:" + TestnameTemp + " 的实验因发生故障已废弃，请稍后重新运行此测试", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information); }));
            }
        ...
        private void RemoveTestList(int pos, int posTwo, bool isDuilte)//add y 20180727  整理testschould列表，将进度中撞针的实验设置为跳过，以实现可以继续剩下的实验
        {
            ...
            string TestnameTemp = testSchedule.TestID.ToString();
            Thread thread = new Thread(new ThreadStart(() => { frmMsgShow.MessageShow("提示", "编号为:" + TestnameTemp + " 的实验因为发生撞针，无法继续，已经排除，请在稍后重新运行此实验"); }));
            //2018-10-16 zlx mod
            foreach (DataGridViewRow dr in dgvWorkListData.Rows)
            {
                if (int.Parse(dr.Cells[1].Value.ToString()) == testSchedule.TestID)
                ...
                if (lisMoveTube.Count > 0)
                {
                    ...
                    if (takepos[0] == "0" && MoveTubeUseFlag == false)//管架取管
                    {
                        if (TubeStop)
                        {
                            ...
                            LogFile.Instance.Write("时间 ：" + DateTime.Now.ToString("HH-mm-ss") + "移除   取管位置 ：" + TempMoveStatus.TakeTubePos + "  放管位置 ：" + TempMoveStatus.putTubePos);
                            continue;
                        }
                        if (putpos[0] == "1")
                        {
                            ...
                            if (!NetCom3.Instance.MoveQuery())
                            {
                                ...
                                if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                                {
                                    ...
                                    {
                                        TubeStop = true;
                                        if ((putpos[1] != "1" || putpos[1] != "2" || putpos[1] != "3") && !AddTubeStop.Contains(int.Parse(putpos[1])))
                                        {
                                            lock (AddTubeStop)
                                            {
                                                AddTubeStop.Add(int.Parse(putpos[1]));
                                    ...
                                    else if (iNeedCool < 11)
                                    {
                                        ...
                                        {
                                            isMoevtoNextBoard = true;
                                            takepos[1] = BoardNextPos(int.Parse(takepos[1]), true, out isMoevtoNextBoard).ToString();
                                        }
                                        else
                                        {
                                            isMoevtoNextBoard = false;
                                            takepos[1] = BoardNextPos(int.Parse(takepos[1]), false, out isMoevtoNextBoard).ToString();
                                        }
                                        //移管手到取管位置takepos[1]（管架）取管
                                        plate = int.Parse(takepos[1]) % 88 == 0 ? int.Parse(takepos[1]) / 88 - 1 : int.Parse(takepos[1]) / 88;//几号板
                                        column = int.Parse(takepos[1]) % 11 == 0 ? int.Parse(takepos[1]) / 11 - (plate * 8) : int.Parse(takepos[1]) / 11 + 1 - (plate * 8);
                                        hole = int.Parse(takepos[1]) % 11 == 0 ? 11 : int.Parse(takepos[1]) % 11;
                                        string num = OperateIniFile.ReadIniData("Tube", "Pos" + (plate + 1), "0", iniPathSubstrateTube);
                                        if (int.Parse(num) == 0)
                                        {
                                            iNeedCool = iNeedCool + 3;
                            ...
                            if (!NetCom3.Instance.MoveQuery())
                            {
                                ...
                                if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在清洗盘夹管到温育盘时多次抓空，实验将进行停止！");
                                        AllStop();
                                    }
                                }
                                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.Sendfailure)
                                ...
                                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在清洗盘夹管到温育盘时发生撞管，实验将进行停止！");
                                        AllStop();
                                    }
                                }
                                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.OverTime)
                                {
                                    setmainformbutten();
                                    NetCom3.Instance.stopsendFlag = true;
                                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在向清洗盘抓新管时接收数据超时！");
                                    DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在清洗盘夹管到温育盘时接收数据超时，实验将进行停止！");
                                    AllStop();
                                }
                                #endregion
                            }
                            ...
                            if (!NetCom3.Instance.MoveQuery())
                            {
                                ...
                                if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                                {
                                    ...
                                    else
                                    {
                                        setmainformbutten();
                                        NetCom3.Instance.stopsendFlag = true;
                                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在清洗盘扔废管时时多次抓空！孔位置" + tubeHoleNum);
                                ...
                                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                                {
                                    ...
                                    else
                                    {
                                        setmainformbutten();
                                        NetCom3.Instance.stopsendFlag = true;
                                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在清洗盘扔废管时时发生撞管！撞管位置" + tubeHoleNum);
                                ...
                                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.OverTime)
                                {
                                    setmainformbutten();
                                    NetCom3.Instance.stopsendFlag = true;
                                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在清洗盘扔废管时时接收数据超时！");
                                    DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在清洗盘扔废管时接收数据超时，实验将进行停止！");
                                    AllStop();
                                }
                                #endregion
                        ...
                        if (!NetCom3.Instance.MoveQuery())
                        {
                            ...
                            if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                            {
                                ...
                                else
                                {
                                    ...
                                    DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在反应盘扔废管时多次抓空，实验将进行停止！");
                                    AllStop();
                                    break;
                                }
                            }
                            ...
                            else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                            {
                                ...
                                else
                                {
                                    setmainformbutten();
                                    NetCom3.Instance.stopsendFlag = true;
                                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在反应盘扔废管时发生撞管！");
                                    DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在反应盘扔废管时发生撞管，实验将进行停止！");
                                    AllStop();
                                }
                            }
                            else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.OverTime)
                            {
                                setmainformbutten();
                                NetCom3.Instance.stopsendFlag = true;
                                LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在反应盘扔废管时接收数据超时！");
                                DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在反应盘扔废管时接收数据超时，实验将进行停止！");
                                AllStop();
                                break;
                            }
                            #endregion
                    ...
                    //    && WashTurnFlag == false && MoveTubeUseFlag == false && WashTrayUseFlag==false)///反应盘标志位是否不用判断
                        ...
                        && MoveTubeUseFlag == false)///反应盘标志位是否不用判断
                    {
                        NoTateFlag = true;
                        MoveTubeUseFlag = true;
                        WashTrayUseFlag = true;
                        ...
                        if (!NetCom3.Instance.MoveQuery())
                        {
                            ...
                            if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                            {
                                ...
                                else
                                {
                                    setmainformbutten();
                                    NetCom3.Instance.stopsendFlag = true;
                                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在反应盘夹管到清洗盘时多次抓空！");
                                    DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在反应盘夹管到清洗盘时多次抓空，实验将进行停止！");
                                    AllStop();
                                }
                            }
                            else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.Sendfailure)
                            ...
                            else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                            {
                                ...
                                else
                                {
                                    setmainformbutten();
                                    NetCom3.Instance.stopsendFlag = true;
                                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在反应盘夹管到清洗盘时发生撞管！撞管位置为" + tubeHoleNum);
                                    //LogFileAlarm.Instance.Write(" *** " + "时间" + DateTime.Now.ToString("HH-mm-ss") + "移管手在反应盘夹管到清洗盘时发生撞管孔位置" + tubeHoleNum + " *** ");
                                    DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在反应盘夹管到清洗盘时发生撞管，实验将进行停止！");
                                    AllStop();
                                }
                            }
                            else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.OverTime)
                            {
                                setmainformbutten();
                                NetCom3.Instance.stopsendFlag = true;
                                LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在反应盘夹管到清洗盘时接收数据超时！");
                                DialogResult tempresult = frmMsgShow.MessageShow("移管手错误", "移管手在反应盘夹管到清洗盘时接收数据超时，实验将进行停止！");
                                AllStop();
                                break;
                            }
                            #endregion
                        ...
                        if (step == "WashTray")
                        {
                            LogFile.Instance.Write("打印清洗步骤越界的问题 ： TempMoveStatus.TestId" + TempMoveStatus.TestId + "\nStepIndex() :" + StepIndex(dgvWorkListData.Rows[TempMoveStatus.TestId - 1].Cells[6].Value.ToString(), "W2") + "\n");
                            lisProBar[TempMoveStatus.TestId - 1].BarColor[StepIndex(dgvWorkListData.Rows[TempMoveStatus.TestId - 1].Cells[6].Value.ToString(), "W2") - 1]
                                   = Color.Gray;
                            toGray(TempMoveStatus.TestId - 1, StepIndex(dgvWorkListData.Rows[TempMoveStatus.TestId - 1].Cells[6].Value.ToString(), "W2") - 1);
                            lisProBar[TempMoveStatus.TestId - 1].Invalidate();

                            BeginInvoke(TestStatusInfo, new object[] { "正在清洗", TempMoveStatus.TestId });
                            BeginInvoke(TestWashInfo, new object[] { washCountNum.ToString(), TempMoveStatus.TestId });
                            //TestWashInfo
                            //dgvWorkListData.Rows[TempMoveStatus.TestId - 1].Cells["SampleNo"].Value = dgvWorkListData.Rows[TempMoveStatus.TestId - 1].Cells["SampleNo"].Value + "-" + currentHoleNum.ToString();
                        ...
                        if (step == "Wash1")
                        {
                            lisProBar[TempMoveStatus.TestId - 1].BarColor[StepIndex(dgvWorkListData.Rows[TempMoveStatus.TestId - 1].Cells[6].Value.ToString(), "W1") - 1]
                                = Color.Gray;
                            toGray(TempMoveStatus.TestId - 1, StepIndex(dgvWorkListData.Rows[TempMoveStatus.TestId - 1].Cells[6].Value.ToString(), "W1") - 1);
                            lisProBar[TempMoveStatus.TestId - 1].Invalidate();
                            //dgvWorkListData.Rows[TempMoveStatus.TestId - 1].Cells["SampleNo"].Value = dgvWorkListData.Rows[TempMoveStatus.TestId - 1].Cells["SampleNo"].Value + "-" + currentHoleNum.ToString();
                            BeginInvoke(TestStatusInfo, new object[] { "正在清洗", TempMoveStatus.TestId });
                            BeginInvoke(TestWashInfo, new object[] { washCountNum.ToString(), TempMoveStatus.TestId });
        ...
        void washStep()
        {
            ...
            if (read == "1")
            {
               ...
               = Color.Gray;
                toGray(int.Parse(dtWashTrayTubeStatus.Rows[24][2].ToString()) - 1, StepIndex(dgvWorkListData.Rows[int.Parse(dtWashTrayTubeStatus.Rows[24][2].ToString()) - 1].
                Cells[6].Value.ToString(), "R"));
            ...
            if (ImbibitionFlag.Contains(1))
            {
                if (ImbibitionFlag[3] == 1 && takeTubeFlag == false)
                {
                    ...
                    toGray(int.Parse(dtWashTrayTubeStatus.Rows[16 + isNewCleanTray][2].ToString()) - 1, StepIndex(dgvWorkListData.Rows[int.Parse(dtWashTrayTubeStatus.Rows[16 + isNewCleanTray][2].ToString()) - 1]
                            .Cells[6].Value.ToString(), "W2"));
                ...
                if (takeTubeFlag)
                {
                    ...
                    lock (locker2)
                    {
                        if (lisMoveTube.Count > 0)
                        {
                            ...
                            LogFile.Instance.Write("时间 ：" + DateTime.Now.ToString("HH-mm-ss") + "  取管位置 ：" + moveTube7.TakeTubePos + "  放管位置 ：" + moveTube7.putTubePos);
                        }
                        else
                        {
            ...
            if (AddSubstrate == "1")
            {
                ...
                toGray(int.Parse(dtWashTrayTubeStatus.Rows[18 + isNewCleanTray][2].ToString()) - 1, StepIndex(dgvWorkListData.Rows[int.Parse(dtWashTrayTubeStatus.Rows[18 + isNewCleanTray][2].ToString()) - 1].
                    Cells[6].Value.ToString(), "Su"));
            ...
            if (dtWashTrayTubeStatus.Rows[28][1].ToString() == "1")
            {
                ...
                lock (locker2)
                {
                    if (lisMoveTube.Count > 0)
                    {
                        ...
                        LogFile.Instance.Write("时间 ：" + DateTime.Now.ToString("HH-mm-ss") + "  取管位置 ：" + moveTube.TakeTubePos + "  放管位置 ：" + moveTube.putTubePos);
                    }
                    else
                    {
        ...
        void Read(object readInfo)
        {
            ...
            while (calNowFlag)
            {
                if (RunFlag == (int)RunFlagStart.IsStoping)
                {
                    calNowFlag = false;
                    return;
                }
            ...
            //CalculatResult(testid, System.Convert.ToInt32("0x" + readData, 16));
            /*
            int testid;
            lock (readLocker)
            {
            ...
            int[] PMTS = {1000, 103999, 102553, 214823, 205634, 408112, 571964, 3696, 3680, 4128, 66632, 52016, 51936, 131384, 130371, 135486, 365370, 366044, 387410, 706779, 688224, 720424, 1665949, 1685689, 1625782 };
            ...
            LogFile.Instance.Write(string.Format("{0}<-:{1}", DateTime.Now.ToString("HH:mm:ss:fff"), "实验" + testid + "读数发光值为:" + PMTS[testid]));
        ...
        void CalculatResult(int testid, int pmt)
        {
            ...
            List<TestItem> lis1 = lisTiEnd.FindAll(tx => tx.SampleNo ==
                dgvWorkListData.Rows[testid - 1].Cells["SampleNo"].Value.ToString());
            ...
            //    int.Parse(dgvWorkListData.Rows[testid - 1].Cells["Position"].Value.ToString()));
            List<TestItem> lis2 = BToListTi.FindAll(tx => tx.SampleNo ==
                dgvWorkListData.Rows[testid - 1].Cells["SampleNo"].Value.ToString());
            DbHelperOleDb db = new DbHelperOleDb(1);
            if (lis1.Count == lis2.Count)
            {
                ...
                for (int i = 0; i < dtSpInfo.Rows.Count; i++)
                {
                    ...
                    try
                    {
                        if (dtSpInfo.Rows[i] == null)
                        {
                            continue;
                        }
                        int length = dtSpInfo.Rows[i]["SampleNo"].ToString().Substring(6).Length;
                        if (int.Parse(dtSpInfo.Rows[i]["SampleNo"].ToString().Substring(6))
                            == int.Parse(dgvWorkListData.Rows[testid - 1].Cells["SampleNo"].Value.ToString().Substring(6, length)))
                        {
            ...
            //查询当前项目范围
            //2018-08-03 zlx mod
            DataTable tbtbProject = DbHelperOleDb.Query(0,@"select ValueUnit,RangeType,VRangeType,ExpiryDate,ValueRange1,ValueRange2 from tbProject where ShortName = '" + ItemName + "'").Tables[0];
            string Range1 = "";
            string Range2 = "";
            int ExpiryDate = 0;
            string VRangeType = "";
            foreach (DataRow dr in tbtbProject.Rows)
            {
                if (dr != null)
                {
                    ...
                    VRangeType = dr["VRangeType"].ToString();//2019-06-06 add
                }
            }
                ...
                else
                {
                    ...
                    if (CurveData.Count > 0)//ltData标准品
                    {
                        ...
                        for (int i = 0; i < CurveData.Count; i++)
                        {
                            ...
                            if (double.IsNaN(CurveData[i].DataValue) || double.IsNaN(CurveData[i].Data))
                            {
                                frmMsgShow.MessageShow("错误", "函数计算错误，可能该数据不适合此回归模型");
                                return;
                            }
                        }
                    ...
                    foreach (double par in er._pars)
                    {
                        ...
                        {
                            //dtScalCacResult.Rows.Add(dtCaculate.Rows[0]["ItemName"].ToString(), 1, "");
                            frmMsgShow.MessageShow("错误", dtCaculate.Rows[0]["ItemName"].ToString() + "项目回归计算时出现运算错误，可能该数据不适合此回归模型");
                            return;
                        }
                    }
        ...
        void AllPause()
        {
            LogFile.Instance.Write("**********" + " 调用了 AllPause() " + "**********");
        ...
        void AllStop()
        {
            LogFile.Instance.Write("**********" + " 调用了 AllStop() " + "**********");
            frmMessageShow frmMsgShow = new frmMessageShow();
            if (RunFlag == (int)RunFlagStart.IsRuning)
            {
                NetCom3.Instance.stopsendFlag = true;
                ...
                MoveTubeUseFlag = AddingSampleFlag = WashTrayUseFlag = false;
                //将清洗盘当前取放管位置的孔号保存到配置文件中
                //OperateIniFile.WriteIniPara("OtherPara", "washCurrentHoleNum", currentHoleNum.ToString());
                IniUpdateAccess();//由配置文件同步试剂与底物信息到数据库
                try
        ...
        void Goon()
        {
            LogFile.Instance.Write("**********" + " 调用了 Goon() " + "**********");
            if (EmergencyFlag || addOrdinaryFlag)
            {
                frmMessageShow frmMsgShow = new frmMessageShow();
        ...
        private void IniUpdateAccess()
        {
            ...
            DbHelperOleDb.ExecuteSql(3,@"update tbSubstrate set leftoverTest =" + sbNum2 + " where BarCode = '"
                                                  + sbCode2 + "'");
            #endregion
            ...
            {
                string DiluteVol = OperateIniFile.ReadIniData("ReagentPos" + dr["DilutePos"], "leftDiuVol", "", iniPathReagentTrayInfo);
                if (DiluteVol != "" && int.Parse(DiluteVol) != int.Parse(dr["DilutePos"].ToString()))
                {
                    dba = new DbHelperOleDb(3);
                    DbHelperOleDb.ExecuteSql(3,@"update tbDilute set LeftDiuVol =" + int.Parse(DiluteVol) + " where DilutePos = " + dr["DilutePos"] + "");
                }
            }
            #endregion
        ...
        private void fbtnAddE_Click(object sender, EventArgs e)
        {
            if (!IsAllowAddSp())
            {
                ...
                frmMessage.MessageShow("温馨提示", "实验即将完成，请在实验完成后继续添加样本！");
                return;
            }
            while (AddingSampleFlag)
            {
        ...
        private void fbtnDelTest_Click(object sender, EventArgs e)
        {
                            ...
                            if (FirstPos == "no4")
                            {
                                for (int num = dtReactTrayInfo.Rows.Count - 1; num > 0; num--)
                                {
                                    if (dtReactTrayInfo.Rows[num][1].ToString() != "0")
                                    {
                                        ...
                                        {
                                            if (num + 1 - 15 > 4)//dtReactTrayInfo.Rows[num + 1 - 15][0].ToString()!= FirstPos
                                                FirstPos = dtReactTrayInfo.Rows[num + 1][0].ToString();
                                        }
                                        break;
                                    }
            ...
            stopTimer.Start();//倒计时继续
            #endregion
            //StopWatchAddOrSubtractEmergency();//加急诊之后增加倒计时,或者减去试验后减少倒计时试验调用了实验
        }
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            testResult.SampleID = 123456;
            testResult.TestID = 654321;
            testResult.SampleNo = "12366996";
            testResult.SampleType = "标准品A";
            testResult.ItemName = "asdfghjklkj";
            testResult.PMT = 987654321;
            testResult.concentration = "asdfghjkl";
            testResult.Result ="不合格";
            testResult.Status =0;
            testResult.ReagentBeach ="zxcvbnm,lfdsjk123456";
            testResult.Unit = "lkjhgf";
        ...
        bool IsAllowAddSp()
        {
            if ((Convert.ToInt32(TimeLabel2.Text) * 60 + Convert.ToInt32(TimeLabel3.Text)) > 5)
            {
                return true;
            }
            else
