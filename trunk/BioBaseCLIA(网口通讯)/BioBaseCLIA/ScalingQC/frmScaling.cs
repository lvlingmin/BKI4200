                    ...
                    if (dt.Rows.Count > 0)
                    {
                        for (int j = 0; j < dt.Rows.Count; j++)
                        {
                            ...
                            dtScalInfo.Rows.Add(shortName, batch,
                                ...
                                ActiveDate, Convert.ToDateTime(dt.Rows[j]["ValidDate"]).ToString("yyyy-MM-dd"), Convert.ToInt32(expiryDate));//2018-11-14 zlx add
                            //dt.Rows[j]["AddDate"]
                        }
                    }
                    else
        ...
        void ShowScalCurve(string itemname, string batch)
        {
            ...
            if (MainltData.Count > 0)//ltData标准品
            {
                ...
                for (int i = 0; i < MainltData.Count; i++)
                {
                    ...
                    {
                        frmMS.MessageShow("错误", "函数计算错误，可能该数据不适合此回归模型");
                        return;
                ...
                foreach (double par in er1._pars)
                {
                    ...
                    {
                        frmMS.MessageShow("错误", "回归计算时出现运算错误，可能该数据不适合此回归模型");
