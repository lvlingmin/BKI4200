    ...
    public partial class frmReagentLoad : frmParent
    {
        ...
        private void btnAddR_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ModelRg.ReagentName = cmbRgName.Text.Trim();
                ModelRg.leftoverTestR1 = int.Parse(txtRgLastTest.Text.Trim());
                if (ModelRg.ReagentName.Contains("TPOAb"))//这里暂时默认一半
                {
                    ModelRg.leftoverTestR2 = int.Parse(txtRgLastTest.Text.Trim())/2;
                    ModelRg.leftoverTestR3 = int.Parse(txtRgLastTest.Text.Trim()) - ModelRg.leftoverTestR2;
                }
                else 
                {
                    ModelRg.leftoverTestR2 = int.Parse(txtRgLastTest.Text.Trim());
        ...
        private void SetDiskProperty()
        {
            ...
            {
                if (Convert.ToInt32(dtRgInfo.Rows[j]["leftoverTestR1"]) < WarnReagent)
                {
                    if (Convert.ToInt32(dtRgInfo.Rows[j]["leftoverTestR1"]) == 0)
                    {
