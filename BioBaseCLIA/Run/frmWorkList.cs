...
namespace BioBaseCLIA.Run
{
    public partial class frmWorkList : frmParent
    {
            ...
            if (RunFlag == (int)RunFlagStart.IsRuning)
            {
                ...
                if (EmergencyFlag)
                {
                    ...
                    int NoStartScalNum = new List<TestItem>((BindingList<TestItem>)this.dgvWorkListData.DataSource).FindAll(ty =>
                        ty.TestID >= NoStartTestId && (ty.SampleType.Contains("校准品")/* || ty.SampleType.Contains("定标液")*/)).Count;
        ...
        public DataTable MethodInit(DataTable dtSampleInfo)
        {
            ...
            for (int i = 0; i < dtSampleInfo.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < int.Parse(dtSampleInfo.Rows[i]["RepeatCount"].ToString()); j++)
                {
                    ...
                    {
                        if (rows[0]["SampleType"].ToString().Contains("标准品") ||
                            rows[0]["SampleType"].ToString().Contains("定标液") ||
                            rows[0]["SampleType"].ToString().Contains("校准品"))
                        {
                            Emergency = "5";
                        }
                        ...
                        if (rows.Length > 0 &&
                            !(rows[0]["SampleType"].ToString().Contains("标准品")
        ...
        void TestRun(object sender, EventArgs e)
        {
            ...
            #region 20201121实验预设 lyq mod
            try
            {
                dtPredictSet = OperateExcel.PreImPortExcel(Environment.CurrentDirectory + "\\仪器调试\\实验预设.xls");

                if (dtPredictSet.Rows[0][1].ToString().Contains("校准品A") || dtPredictSet.Rows[0][1].ToString().Contains("标准品A"))
                {
                    predictType = 0;
                }
                else if (dtPredictSet.Rows[0][1].ToString().Contains("校准品C") || dtPredictSet.Rows[0][1].ToString().Contains("标准品C"))
                {
                    predictType = 1;
                }
                else if (dtPredictSet.Rows[0][1].ToString().Contains("定标液"))
                {
                    predictType = 2;
                }

                int spNum = dtSpInfo.Rows.Count;
                prePara = spNum;
                for (int i = 0; i < dtPredictSet.Rows.Count; i++)
                {
                    if (dtPredictSet.Rows[i][0].ToString() == spNum.ToString())
                    {
                        //DataRow[] ddr = dtPredictSet.Select();
                        int loopNum = dtPredictSet.Rows.Count;
                        for (int j = loopNum - 1; j >= 0; j--)
                        {
                            if (dtPredictSet.Rows[j][0].ToString() != spNum.ToString())
                            {
                                dtPredictSet.Rows.RemoveAt(j);
                            }
                        }
                        spNum = 10000;
                        break;
                ...
                if (dtPredictSet.Rows[0][1].ToString().Contains("校准品A") || dtPredictSet.Rows[0][1].ToString().Contains("标准品A") || dtPredictSet.Rows[0][3].ToString() == "")
                {
                    predictType = 0;
                }
                else if (dtPredictSet.Rows[0][1].ToString().Contains("校准品C") || dtPredictSet.Rows[0][1].ToString().Contains("标准品C"))
                ...
                else if (dtPredictSet.Rows[0][1].ToString().Contains("定标液") || dtPredictSet.Rows[0][3].ToString() != "")
                {
                    predictType = 2;
                }
                ...
                if (predictType == 2)
                {
                    ...
                    bllscalResult.Add(modelScalingResult);

                }
                if (spNum != 10000)
                {
                    frmMsgShow.MessageShow("", "请检查配置文件！");
                    //释放已经开启的移管线程
            ...
            try
            {
                prePara = double.Parse(OperateIniFile.ReadInIPara("PrePara", "pre" + prePara));
            }
            catch (System.Exception ex)
            {
                prePara = 0.7;
        ...
        private bool CleanTube()
        {
            if (!washTrayTubeClear(null))
            {
                MessageBox.Show("清空清洗盘故障，请检查仪器！", "温馨提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            if (RunFlag != (int)RunFlagStart.IsRuning)
            {
                MessageBox.Show("实验状态故障，请检查运行状态！", "温馨提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            if (!CleanTrayWashPipeline())
            {
                MessageBox.Show("清洗故障，请检查清洗装置运行状况！", "温馨提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            if (!reactTrayTubeClear(null))
            {
                MessageBox.Show("清空温育盘故障，请检查温育盘是否异常！", "温馨提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            if (RunFlag != (int)RunFlagStart.IsRuning)
            {
            ...
            foreach (var item in ItemNames)
            {
                ...
                */
                ...
                if (itemRiInfo.Count > 0)
                {
                    ...
                    List<TestItem> ScaSameItem = lisSameItem.FindAll(ty => (ty.SampleType.Contains("校准品")
                    ...
                    foreach (var reBNum in regenBatchNums)
                    {
                        ...
                        if (int.Parse(dtItemInfo.Rows[0][0].ToString()) == 1)
                        {
                            ...
                            for (int i = 0; i < int.Parse(dtItemInfo.Rows[0][2].ToString()); i++)
                            {
                                ...
                                {
                                    case 0:
                                        dtScal.Rows.Add("校准品A", lisItem.FindAll(ty => ty.SampleType == "校准品A"
                                            && ty.RegentBatch == reBNum.Key).Count);
                                        break;
                                    case 1:
                                        dtScal.Rows.Add("校准品B", lisItem.FindAll(ty => ty.SampleType == "校准品B"
                                            && ty.RegentBatch == reBNum.Key).Count);
                                        break;
                                    case 2:
                                        dtScal.Rows.Add("校准品C", lisItem.FindAll(ty => ty.SampleType == "校准品C"
                                        && ty.RegentBatch == reBNum.Key).Count);
                                        break;
                                    case 3:
                                        dtScal.Rows.Add("校准品D", lisItem.FindAll(ty => ty.SampleType == "校准品D"
                                        && ty.RegentBatch == reBNum.Key).Count);
                                        break;
                                    case 4:
                                        dtScal.Rows.Add("校准品E", lisItem.FindAll(ty => ty.SampleType == "校准品E"
                                        && ty.RegentBatch == reBNum.Key).Count);
                                        break;
                                    case 5:
                                        dtScal.Rows.Add("校准品F", lisItem.FindAll(ty => ty.SampleType == "校准品F"
                                        && ty.RegentBatch == reBNum.Key).Count);
                                        break;
                                    case 6:
                                        dtScal.Rows.Add("校准品G", lisItem.FindAll(ty => ty.SampleType == "校准品G"
                                        && ty.RegentBatch == reBNum.Key).Count);
                                        break;
                                }
                            }
                            ...
                            if (sNum > 0)//六个标准品中存在1个个数为0
                            {
                                if (int.Parse(dtScal.Rows[2][1].ToString()) > 0 && int.Parse(dtScal.Rows[4][1].ToString()) > 0)//C.E两点数量不为0
                                {
                                    ...
                                    if (ExitsMainCurve) //有主曲线
                                    {
                                        ...
                                        if (lisSameItem.FindAll(ty => (ty.SampleType.Contains("校准品"))).Count == 0)
                                        {
                                            #region 判断定标曲线是否可以使用
        ...
        private void addLiquid(object testS)
        {
            ...
            switch (testTempS.TestScheduleStep)
            {
                                ...
                                for (int i = 0; i < diuList.Count; i++)
                                {
                                    ...
                                    if (rgPos > 0)
                                    {
                                        ...
                                        if (NetCom3.Instance.LiquidLevelDetectionFlag == (int)LiquidLevelDetectionAlarm.Low &&
                                            ...
                                            (!(dgvWorkListData.Rows[testTempS.TestID - 1].Cells["SampleType"].Value.ToString().Contains("校准品"))))
                        ...
                        else if (TestStepSingle[j] == "S")
                        {
                            ...
                            else
                            {
                                ...
                                if (AddErrorCount > 0)
                                {
                                    ...
                                    {
                                        NetCom3.Instance.stopsendFlag = true;
                                        ShowWarnInfo("加样针加样本时撞针未能修复", "加样", 1);
                        ...
                        else if (TestStepSingle[j] == "R1")
                        {
                            ...
                            if (drRg.Length > 0)
                            {
                                for (int g = 0; g < drRg.Length; g++)
                                {
                                    ...
                                    if (dgvWorkListData.Rows[testTempS.TestID - 1].Cells["SampleType"].Value.ToString().Contains("校准品"))
                                    {
                ...
                case TestSchedule.ExperimentScheduleStep.AddBeads:
                    ...
                    //LogFile.Instance.Write(string.Format("\n时间 :testTempS==null ? :" + (testTempS == null),
                    //    DateTime.Now.ToString("HH:mm:ss:fff"))); ;
                    //LogFile.Instance.Write(string.Format("\n" +"testTempS.TestID:" + testTempS.TestID ));
                    //LogFile.Instance.Write(string.Format("\n" + "dgvWorkListData.Rows:" + testTempS.TestID));
                    //LogFile.Instance.Write(string.Format("\n" + "testTempS.TestID:" + testTempS.TestID));
                    //LogFile.Instance.Write(string.Format("\n" + "dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString():" + 
                    //    dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString()));
                    //LogFile.Instance.Write(string.Format("\n" + "StepIndex:"+ StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "B")));
                    #endregion
                    try
                    {
        ...
        void rackToReact(int ReactPos)
        {
            ...
            if (firstTubeIndex < 0)
            {
                setmainformbutten();
                TubeStop = true;
                LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "异常" + " *** " + "未读" + " *** " + "暂存盘为空，将停止加样！");
            }
            ...
            if (!NetCom3.Instance.MoveQuery())
            {
                ...
                if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                {
                    ...
                    else
                    {
                        ...
                        dbtnRackStatus();
                        setmainformbutten();
                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手向温育盘抓新管多次抓空!实验暂停加样！");
                ...
                for (int j = starPos; j < lisMoveTube.Count; j++)//从某个位置开始
                {
                    ...
                    if (TakeTubeAtring[0] == "0")
                    {
                        ...
                        if (pos > 352)
                        {
                            ...
                            {
                                AllPause();
                                MessageBox.Show("缺少反应管，实验已暂停，请装填暂存盘，然后稍后重新开始", "警告", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                pos = 1;
                            }
                            else pos = tary * 88 + 1;
                        }
        ...
        void Read(object readInfo)
        {
            ...
            if (predictType == 0)//6点
            {
                ...
                int spNum = dtSpInfo.Rows.Count;
                //根据testid获得位号
                //string tempPosition = lisTestMethod.Select("TestID='" + testid +"'")[0]["SamplePos"].ToString();
                //string tempPosition = lisTestMethod.Find(xy => xy.TestID == testid).SamplePos.ToString();
                string tempPosition = dgvWorkListData.Rows[testid - 1].Cells["Position"].Value.ToString();
                //根据位号加载 标准品的预设
                if (int.Parse(tempPosition) <= 7 && dtPredictSet.Rows[int.Parse(tempPosition) - 1][1].ToString().Contains("校准品"))
                {
                    if (dtPredictSet.Rows[int.Parse(tempPosition) - 1][3].ToString() != null)//如果发光值不空
                    {
                        Random ra = new Random();
                        //pmt = int.Parse(dtPredictSet.Rows[int.Parse(tempPosition) - 1][3].ToString()) + ra.Next(500, 5000);
                        if (ra.Next(1, 3) == 1)//+
                        {
                            pmt = int.Parse(dtPredictSet.Rows[int.Parse(tempPosition) - 1][3].ToString());
                            pmt += ra.Next(pmt / 200, pmt * 3 / 100);
                        }
                        else//-
                        {
                            pmt = int.Parse(dtPredictSet.Rows[int.Parse(tempPosition) - 1][3].ToString());
            ...
            else if (predictType == 1)//2点
            {
                ;
            }
            else if (predictType == 2)//定标液
            {
                //根据testid获得位号
                string tempPosition = dgvWorkListData.Rows[testid - 1].Cells["Position"].Value.ToString();
                //读取预设，读dt[0][1],dt[0][1]移动到后面dt[0][2]到dt[0][1]位置
                if (int.Parse(tempPosition) == 1 && dtPredictSet.Rows[/*int.Parse(tempPosition) - 1*/ 0][1].ToString().Contains("定标液"))
                {
                    //Random ra = new Random();
                    //pmt = int.Parse(dtPredictSet.Rows[int.Parse(tempPosition) - 1][3].ToString()) + ra.Next(500, 5000);

                    pmt = int.Parse(dtPredictSet.Rows[0][2].ToString());
                    Random ra = new Random();
                    if (ra.Next(1, 3) == 2)
                    {
                        pmt = pmt + ra.Next(50, (int)((double)pmt * 0.05));
                    }
                    else
                    {
                        pmt = pmt - ra.Next(50, (int)((double)pmt * 0.05));
                    ...
                    //切换位置
                    int preCalRepeat = 1;
                    while (dtPredictSet.Rows[0][preCalRepeat + 1].ToString() != "")
                    {
                        preCalRepeat++;
                    }

                    string temp = dtPredictSet.Rows[0][2].ToString();
                    for (int i = 0; i < preCalRepeat - 1; i++)
                    {
                        dtPredictSet.Rows[0][i + 2] = dtPredictSet.Rows[0][i + 3];
                    }
                    dtPredictSet.Rows[0][preCalRepeat + 1] = temp;
        ...
        void CalculatResult(int testid, int pmt)
        {
            ...
            else
            {
                ...
                if (sampleType.Contains("校准品"))
                {
                    if (CurrentScal.Num.Split(',').Length >= 6)
                    {
                        ...
                        else if (sampleType == "校准品B")
                        {
                            conc = double.Parse(concs[1]);
                        }
                        else if (sampleType == "校准品C")
                        {
                            conc = double.Parse(concs[2]);
                        }
                        else if (sampleType == "校准品D")
                        {
                            conc = double.Parse(concs[3]);
                        }
                        else if (sampleType == "校准品E")
                        {
                            conc = double.Parse(concs[4]);
                        }
                        else if (sampleType == "校准品F")
                        {
                            conc = double.Parse(concs[5]);
                        }
                        else if (sampleType == "校准品G")
                        {
                ...
                else if (sampleType.Contains("定标液"))
                {
                    ;
                }
                else
                {
                    ...
                    #region 实验预设20201121 lyq mod
                    DataTable scaResult = DbHelperOleDb.Query(1, @"select Points,ActiveDate from tbScalingResult where ItemName = '" + ItemName
                    + "' and RegentBatch = '" + Batch + "' and Status = 1").Tables[0];
                    string points = "";
                    foreach (DataRow dr in scaResult.Rows)
                    {
                        if (dr != null)
                        {
                            points = dr["Points"].ToString();
                    ...
                    string[] preSca = points.Replace("(", "").Replace(")", "").Split(';');
                    if (predictType == 0)
                    {
                        //获取位号
                        int tempPosition = int.Parse(dgvWorkListData.Rows[testid - 1].Cells["Position"].Value.ToString());
                        double tempCon = double.Parse(dtPredictSet.Rows[tempPosition - 1][2].ToString());

                        Random ra = new Random();
                        //double tempRa = ra.NextDouble();

                        #region 非永久性注释
                        //if (tempCon == 1)
                        //{
                        //    #region
                        //    if (ra.Next(1, 11) % 2 == 0)//执行加法
                        //    {
                        //        while (tempRa > 0.7)
                        //        {
                        //            tempRa = ra.NextDouble();
                        //        }
                        //        tempCon = tempCon + tempRa;
                        //    }
                        //    else//执行减法
                        //    {
                        //        while (tempRa > 0.1)
                        ...
                        if (tempCon < 10000)
                        {
                            ...
                            tempCon *= mul;
                            min = (int)tempCon / 200;
                            max = ((int)tempCon / 100) * 5;
                            raSum = (double)ra.Next(min, max) / mul;
                            tempCon /= mul;
                        }
                        if (tempCon > 190 && tempCon < 200)//单独为XX线性顶点设置
                        {
                            if ((tempCon + raSum) >= 200 || (tempCon - raSum) <= 190)
                                goto reset;
                        }
                        if (ra.Next(1, 3) == 1)//+
                        {
                            tempCon = tempCon + raSum;
                        }
                        else//-
                        {
                            tempCon = tempCon - raSum;
                        ...
                        int mainCount = 6;
                        double[] nongdu = new double[mainCount - 1];
                        double[] faguagnzhi = new double[mainCount - 1];

                        for (int i = 0; i < 6; i++)//想要预测的数据
                        {
                            data_Values.Add(new Data_Value()
                            {
                                Data = Convert.ToDouble(double.Parse(preSca[i].Split(',')[0])),
                                DataValue = Convert.ToDouble(double.Parse(preSca[i].Split(',')[1]))
                            });
                            if (i == 0)
                                continue;
                            nongdu[i - 1] = Convert.ToDouble(double.Parse(preSca[i].Split(',')[0]));
                            faguagnzhi[i - 1] = Convert.ToDouble(double.Parse(preSca[i].Split(',')[1]));
                        }
                        if (tempCon > double.Parse(preSca[5].Split(',')[0]))//阳参的发光值
                        {
                            pmt = int.Parse(dtPredictSet.Rows[tempPosition - 1][3].ToString());
                            if (ra.Next(1, 3) == 1)
                                pmt += ra.Next(pmt / 1000, pmt * 3 / 100);
                            else
                                pmt -= ra.Next(pmt / 1000, pmt * 3 / 100);
                        }
                        else if ((double.Parse(preSca[0].Split(',')[0]) <= tempCon && tempCon <= double.Parse(preSca[1].Split(',')[0])))//AB两点中间的发光值
                        {
                            double resultLine = GetLineResultSandwich(tempCon, data_Values);
                            pmt = (int)Math.Ceiling(resultLine < data_Values[0].Data ? (data_Values[0].Data + 0.001) : resultLine);
                        }
                        else//主要的发光值
                        {
                            ...
                            double resultSandwich = GetResultSandwich(tempCon, canshu);
                            pmt = (int)Math.Ceiling(resultSandwich);
                        }
                        concentration = tempCon.ToString();
                    ...
                    else if (predictType == 2)
                    {
                        //获取位号
                        int tempPosition = int.Parse(dgvWorkListData.Rows[testid - 1].Cells["Position"].Value.ToString());
                        concentration = dtPredictSet.Rows[tempPosition - 1][2].ToString();

                        pmt = int.Parse(dtPredictSet.Rows[tempPosition - 1][3].ToString());

                        #region 波动
                        if (testid == 1 && !dgvWorkListData.Rows[1].Cells["SampleType"].Value.ToString().Contains("定标液"))
                        {
                            preSum = double.Parse(OperateIniFile.ReadInIPara("PrePara", "preSum"));
                        }
                        if (preSum != 0)
                        {
                            preCutoff = preSum / 3 * prePara;
                            OperateIniFile.WriteIniPara("PrePara", "preSum", preSum.ToString());
                            preSum = 0;
                        }
                        Random ra = new Random();
                        int min = 50, max = /*500*/ (int)((double)pmt * 0.05);
                        //if(dgvWorkListData.Rows[testid - 1].Cells["SampleType"].Value.ToString().Contains("精密度"))
                        //{
                        //    max = 50000;
                        //}
                        if (ra.Next(1, 3) == 2 /*|| pmt < 100000*/)//+
                        {
                            pmt += ra.Next(min, max);
                        }
                        else//-
                        ...
                        concentration = ((double)pmt / preCutoff).ToString(); ;
                        #endregion

                        //string a = dtPredictSet.Rows[tempPosition - 1][2].ToString();LogFile.Instance.Write("a--------" + a);
                        //string b = dtPredictSet.Rows[tempPosition - 1][3].ToString(); LogFile.Instance.Write("b--------" + b);
                        //string c = dtPredictSet.Rows[tempPosition - 1][4].ToString(); LogFile.Instance.Write("c--------" + c);
                        //string d = dtPredictSet.Rows[tempPosition - 1][5].ToString(); LogFile.Instance.Write("d--------" + d);
                        //string e = dtPredictSet.Rows[tempPosition - 1][6].ToString(); LogFile.Instance.Write("e--------" + e);
                        //string f = dtPredictSet.Rows[tempPosition - 1][7].ToString(); LogFile.Instance.Write("f--------" + f);

                        //切换位置
                        int preCalRepeat = 1;
                        while (dtPredictSet.Rows[tempPosition - 1][preCalRepeat + 1].ToString() != "")
                        {
                            if (dtPredictSet.Rows[tempPosition - 1][preCalRepeat + 1].ToString() == "-")
                            {
                                //result
                                result = "-";
                                break;
                            }
                            else if (dtPredictSet.Rows[tempPosition - 1][preCalRepeat + 1].ToString() == "+")
                            {
                                //result
                                result = "+";
                                break;
                        ...
                        preCalRepeat = preCalRepeat / 2;

                        string tempCon = dtPredictSet.Rows[tempPosition - 1][2].ToString();
                        string tempPmt = dtPredictSet.Rows[tempPosition - 1][3].ToString();
                        for (int i = 0; i < preCalRepeat - 1; i++)
                        {
                            //con
                            dtPredictSet.Rows[tempPosition - 1][(i + 1) * 2] = dtPredictSet.Rows[tempPosition - 1][(i + 2) * 2];
                            //pmt
                            dtPredictSet.Rows[tempPosition - 1][(i + 1) * 2 + 1] = dtPredictSet.Rows[tempPosition - 1][(i + 2) * 2 + 1];

                        }
                        dtPredictSet.Rows[tempPosition - 1][preCalRepeat * 2] = tempCon;
                        dtPredictSet.Rows[tempPosition - 1][preCalRepeat * 2 + 1] = tempPmt;
                    }
                    ...
                    if (predictType < 2)
                    {
                        ...
                        if (double.Parse(concentration) > double.Parse(preSca[5].Split(',')[0]))
                        {
                            concentration = ">" + preSca[5].Split(',')[0];
                        }
                    }
                    //else if(predictType == 1)
                    //{
            ...
            if (concentration != "" && !(concentration.Contains(">") || concentration.Contains("<")))
                testResult.concentration = Convert.ToDouble(concentration).ToString("F" + 3 + "");//2018-11-07 zlx mod
            else
                testResult.concentration = concentration;
        ...
        void SaveTestResultData(List<TestItem> BToListTi, List<TestItem> lis1, List<TestItem> lis2)
        {
            ...
            #region 20201121实验预设 lyq mod
            if (testResult.SampleType.Contains("校准品") || testResult.SampleType.Contains("标准品") || testResult.SampleType.Contains("定标液"))
            {
                testResult.concentration = "";
            }
            #endregion

            BTestResult.Add(testResult);
            TemporaryTestResult.Add(testResult);
            if (testResult.SampleType.Contains("校准品"))
            {
                GC.KeepAlive(testResult);//防止被回收               
                List<TestItem> BToList = BToListTi.FindAll(tx => (tx.ItemName == testResult.ItemName && tx.SampleType.Contains("校准品") && tx.RegentBatch == dgvWorkListData.Rows[testResult.TestID - 1].Cells["RegentBatch"].Value.ToString()));
                List<TestItem> ENDList = lisTiEnd.FindAll(tx => (tx.ItemName == testResult.ItemName && tx.SampleType.Contains("校准品")));
                if (BToList.Count == ENDList.Count)
                {
                    ...
                    List<TestResult> ScalingResult = new List<TestResult>(TemporaryTestResult).FindAll(tx => (tx.ItemName == testResult.ItemName && testResult.SampleType.Contains("校准品")));
        ...
        public DataTable GetNoAddLiquid()
        {
            ...
            for (int i = 0; i < ItemNames.Count; i++)
            {
                ...
                foreach (TestSchedule test in ItemNamelist)
                {
                    ...
                    if (!drRunInfo[0]["SampleType"].ToString().Contains("校准品") && !drRunInfo[0]["SampleType"].ToString().Contains("质控品"))
                    {
        ...
        bool AllowDel()
        {
            ...
            {
                if (dgvWorkListData.SelectedRows[i].Cells["SampleType"].Value.ToString().Contains("校准品")
                    || dgvWorkListData.SelectedRows[i].Cells["SampleType"].Value.ToString().Contains("定标液"))
                {
                    frmMsgShow.MessageShow("工作列表", "选中的实验中存在校准品，请重新选择！");
                    return false;
                }
        ...
        public double GetResultSandwich(double xValue, double[] parameters)
        {
            ...
            if (xValue < 0)
                xValue = 0;
            return (parameters[0] - parameters[3]) / (1 + Math.Pow(xValue / parameters[2], parameters[1])) + parameters[3] * parameters[4];//(parameters[0] - parameters[3]) / (1 + Math.Pow(xValue / parameters[2], parameters[1])) + parameters[3];
