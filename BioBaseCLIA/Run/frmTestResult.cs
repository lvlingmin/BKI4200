        ...
        public void SaveStandardResult(List<TestResult> listResult)
        {
            ...
            for (int j = 0; j < UsingItemNew.Rows.Count; j++)
            {
                ...
                List<TestResult> FilistStandardResult = listResult.FindAll(ty => ty.ItemName == UsingItemNew.Rows[j][0].ToString()
                                                                          && ty.SampleType.Contains("校准品") && ty.ReagentBeach == UsingItemNew.Rows[j][1].ToString());
                ...
                */
                ...
                {
                    if (item.SampleType == "校准品C") hasPointerC = true;
                    if (item.SampleType == "校准品E") hasPoniterE = true;
                }
                ...
                {
                    frmMsgShow.MessageShow("实验结果", "校准品数据不足");
                    continue;
                }
                else
                {
