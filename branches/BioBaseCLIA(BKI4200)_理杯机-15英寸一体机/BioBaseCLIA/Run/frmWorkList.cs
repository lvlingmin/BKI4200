...
namespace BioBaseCLIA.Run
{
    public partial class frmWorkList : frmParent
    {
        ...
        {
            LogFile.Instance.Write("时间" + DateTime.Now);
            ...
            #region 数据库连接变量
            BLL.tbProject bllproject = new BLL.tbProject();
            DbHelperOleDb db1 = new DbHelperOleDb(0);
            DataTable dtproinfo = bllproject.GetList("").Tables[0];

            BLL.tbQC tbQC = new BLL.tbQC();
            db1 = new DbHelperOleDb(3);
            DataTable dtqc = tbQC.GetAllList().Tables[0];

            db1 = new DbHelperOleDb(3);
            DataTable dtreagent = new BLL.tbReagent().GetAllList().Tables[0];
            db1 = new DbHelperOleDb(3);
            DataTable dtdilute = new BLL.tbDilute().GetAllList().Tables[0];

            db1 = new DbHelperOleDb(1);
            BLL.tbScalingResult tbScaling = new BLL.tbScalingResult();
            DataTable dtscaling = tbScaling.GetAllList().Tables[0];

            db1 = new DbHelperOleDb(1);
            BLL.tbMainScalCurve tbMain = new BLL.tbMainScalCurve();
            DataTable dtMain = tbMain.GetAllList().Tables[0];
            #endregion 
            ...
            foreach (var item in ItemNames)
            {
                ...
                if (list.Count > 0)
                {
                    DataRow[] dataRows = dtproinfo.Select("ShortName = '" + item.Key + "'");
                    string DiluteCount = dataRows[0]["DiluteCount"].ToString();
                    string DiluteName = dataRows[0]["DiluteName"].ToString();
                    int DiuVolLeftCount = 0;
                    LogFile.Instance.Write("时间1" + DateTime.Now);
                    foreach (ReagentIniInfo RiInfo in itemRiInfo)
                    {
                        DataRow[] reagentrows = dtreagent.Select("BarCode='" + RiInfo.BarCode + "'");
                        string pos = reagentrows[0]["Postion"].ToString();
                        DataRow[] diluterows = dtdilute.Select("DilutePos=" + int.Parse(pos) + "");

                        if (diluterows.Length > 0)
                        {
                            string ValiData = diluterows[0]["ValiData"].ToString();
                ...
                db = new DbHelperOleDb(0);
                DataRow[] projectrows = dtproinfo.Select("ShortName = '" + item.Key + "'");
                ...
                if (regenBatchNums.Count > 0)
                {
                    foreach (var reBNum in regenBatchNums)
                    {
                        ...
                        dtScal.Clear();
                        db = new DbHelperOleDb(1);
                        DataRow[] scalingrows = dtscaling.Select("ItemName = '"
                        + item.Key + "'and RegentBatch = '" + reBNum.Key + "' and status = 1");
                        string points = null;
                        string ActiveDate = null;
                        foreach (DataRow dr in scalingrows)
                        {
                        ...
                        LogFile.Instance.Write("时间b1" + DateTime.Now);
                        if (projectrows[0]["ProjectType"].ToString().Trim() == "1")
                        {
                            #region 定量实验
                            for (int i = 0; i < int.Parse(projectrows[0]["CalPointNumber"].ToString()); i++)
                            {
                                switch (i)
                                {
                                    case 0:
                            ...
                            if (sNum > 0)//六个标准品中存在1个个数为0
                            {
                                ...
                                {
                                    DataRow[] dataRows = dtMain.Select("ItemName='" + item.Key + "' and RegentBatch='" + reBNum.Key + "'");
                                    if (dataRows.Length > 0) //有主曲线
                                    {
                                        ...
                                        if (lisSameItem.FindAll(ty => (ty.SampleType.Contains("标准品"))).Count == 0)
                                        {
                                            ...
                                            {
                                                DataRow[] dataRows2 = dtMain.Select("ItemName='" + item.Key + "' and RegentBatch='" + reBNum.Key + "'");
                                                if (dataRows.Length > 0)
                                                {
                                                    string[] mainPoint = dataRows2[0]["Points"].ToString().Split(';');
                                        ...
                                        scalingInfo.TestConc = projectrows[0]["CalPointConc"].ToString().Split(',')[2] + ","
                                            + projectrows[0]["CalPointConc"].ToString().Split(',')[4];
                                        scalingInfo.testType = int.Parse(projectrows[0]["ProjectType"].ToString());
                                    ...
                                    else//无主曲线
                                    {
                                        ...
                                        else if (DateTime.Now.Date.AddDays(-Convert.ToInt32(projectrows[0]["ExpiryDate"])).Date > Convert.ToDateTime(ActiveDate))
                                        {
                                        ...
                                        else
                                        {
                                            ...
                                            scalingInfo.RegenBatch = reBNum.Key;
                                            scalingInfo.Num = "0";
                                            scalingInfo.TestConc = projectrows[0]["CalPointConc"].ToString();
                                            scalingInfo.testType = int.Parse(projectrows[0]["ProjectType"].ToString());
                                            lisScalingInfo.Add(scalingInfo);
                                            scalingInfo = new ScalingInfo();
                                        }
            ...
            string ValidDate1 = OperateIniFile.ReadIniData("Substrate1", "ValidDate", "", iniPathSubstrateTube);
            ...
            LogFile.Instance.Write("时间d" + DateTime.Now);
            foreach (TestItem item in QCList)
            {
                ...
                DataRow[] qcdataRows = dtqc.Select("Status = '1' and ProjectName = '" + item.ItemName + "'and QCLevel = '" + QCLevel + "'");
                if (qcdataRows == null || qcdataRows.Length == 0)
                {
            ...
            try
            {
                if (lisTestSchedule.Count > 0)
                {
                        ...
                        && RunFlag == (int)RunFlagStart.IsRuning)
                    {
                        ...
                        {
                            if ((!frmMain.StopFlag[0] && !frmMain.StopFlag[1] && !frmMain.StopFlag[2] && !frmMain.StopFlag[3]) && StopList.Count > 0 && !TubeStop && !SubstrateStop && !BFullReactTray)
                            {
        ...
        private void addLiquid(object testS)
        {
            ...
            switch (testTempS.TestScheduleStep)
            {
                ...
                case TestSchedule.ExperimentScheduleStep.AddBeads:
                    ...
                    break;
                case TestSchedule.ExperimentScheduleStep.AddSingleR:
                    if (testTempS.singleStep == "R2" && testTempS.ItemName.Contains("TPOAb") &&
                        int.Parse(OperateIniFile.ReadIniData("ReagentPos" + dgvWorkListData.Rows[testTempS.TestID - 1].Cells["RegentPos"].Value.ToString(), "LeftReagent2", "", iniPathReagentTrayInfo)) == 0)
                        testTempS.singleStep = "R2ToR3";
                    ...
                    else if (testTempS.singleStep == "R3")
                    {
                        ...
                        BeginInvoke(TestStatusInfo, new object[] { "正在加试剂", testTempS.TestID });
                        lisProBar[testTempS.TestID - 1].BarColor[StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "R3")]
                            = Color.Yellow;
                        lisProBar[testTempS.TestID - 1].Invalidate();
                        #region 加R过程
                    ...
                    else if (testTempS.singleStep == "R2ToR3")
                    {
                        lisProBar[testTempS.TestID - 1].BarColor[StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "R2") - 1]
                        = Color.Gray;
                        lisProBar[testTempS.TestID - 1].Invalidate();
                        ...
                        BeginInvoke(TestStatusInfo, new object[] { "正在加试剂R2", testTempS.TestID });
                        lisProBar[testTempS.TestID - 1].BarColor[StepIndex(dgvWorkListData.Rows[testTempS.TestID - 1].Cells[6].Value.ToString(), "R2")]
                            = Color.Yellow;
                        lisProBar[testTempS.TestID - 1].Invalidate();
                        #region 加R过程
