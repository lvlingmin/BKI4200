...
namespace BioBaseCLIA
{
    ...
    public partial class frmMain : Form
    {
        ...
        // 缺液3分钟暂停加样
        private const int MaxBuffertime = 3;//磁珠清洗液报警最大次数
        private const int MaxWashtime = 3;//探针清洗液报警最大次数
        private const int MaxWastetime = 3;//废液报警最大次数
        private const int MaxWTubetime = 3;//废管报警最大次数
        ...
        private void Work()
        {
            ...
            if (communicationType.Contains("NetConn") || communicationType.Contains("网口通讯"))
            {
                ...
                {
                    frmMsgShow.MessageShow(Getstring("keywordText.ConnectSet"), Getstring("keywordText.LisSelect"));
                    return;
                }

                if (Port.Trim() == "")
                {
                    frmMsgShow.MessageShow(Getstring("keywordText.ConnectSet"), Getstring("keywordText.NullPort"));
                    return;
                }

                if (!Inspect.InspectIP(IPAddress.Trim()))
                {
                    frmMsgShow.MessageShow(Getstring("keywordText.ConnectSet"), Getstring("keywordText.IPErrorMesage"));
                    return;
                }
        ...
        public void ShowLiquidInfo()
        {
            ...
            if (LackLq[3] > 0)
            {
                ...
                if (LackLq[3] > MaxWTubetime)
                {
                    ...
                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + GetString("keywordText.Error") + " *** " + GetString("keywordText.NotRead") + " *** " +GetString("keywordText.Wastetubefull") );
                    LogBtnColorChange(0);
                    StopFlag[3] = true;
                }
                else
                {
                    ...
                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + GetString("keywordText.Warning") + " *** " + GetString("keywordText.NotRead") + " *** " + GetString("keywordText.Wastetubefull"));
            ...
            try
            {
                if (NetCom3.Instance.CheckMyIp_Port_Link())
                {
...

                    {
                        ...
                        frmMS.MessageShow(GetString("keywordText.Tips"), GetString("keywordText.Unableconnect"));
                        frmMS.Dispose();
                        return;
                    }
                    else
        ...
        private void dbtnRack_Click(object sender, EventArgs e)
        {
            if (LackTube[0] == 0|| LackTube[1]==0)
            {
                ...
                new Thread(new ParameterizedThreadStart((obj) =>
                {
                    SetCultureInfo();
                    frmMessageShow f = new frmMessageShow();
                    f.MessageShow(GetString("keywordText.Tips"), Err);
                }))
                { IsBackground = true, CurrentCulture = Language.AppCultureInfo, CurrentUICulture = Language.AppCultureInfo }.Start();
            }
            else
